#!/usr/bin/env python3
"""SnapForge — first-time setup script.
...
def create_default_admin():
    ...
    try:
        ...
        username = os.getenv("SNAPFORGE_ADMIN_USER", "").strip().lower()
        password = os.getenv("SNAPFORGE_ADMIN_PASSWORD", "").strip()

        if username and password:
            # Both provided via env — validate before using
            if username in RESERVED_USERNAMES:
                print(f"  [error] SNAPFORGE_ADMIN_USER '{username}' is a reserved username")
                return "failed"
            if len(password) < PASSWORD_MIN_LENGTH:
                print(f"  [error] SNAPFORGE_ADMIN_PASSWORD must be at least {PASSWORD_MIN_LENGTH} characters")
                return "failed"
        elif sys.stdin.isatty() and not os.getenv("SNAPFORGE_SKIP_ADMIN_PROMPT"):
        ...
        if sys.stdin.isatty() and not os.getenv("SNAPFORGE_ADMIN_PASSWORD"):
            print(f"  [ok] Admin account created ({username})")
        else:
            print(f"  [ok] Initial admin user created ({username})")
            if not os.getenv("SNAPFORGE_ADMIN_PASSWORD"):
                print(f"        Temporary password: {password}")
                print(f"        ** Change it after first login. Set SNAPFORGE_ADMIN_PASSWORD to choose your own. **")
        return "created"
...
def main():
    ...
    # Load .env so pre-seeded SNAPFORGE_ADMIN_USER / SNAPFORGE_ADMIN_PASSWORD (and
    ...
    if not os.getenv("SNAPFORGE_SKIP_RUN_HINT"):

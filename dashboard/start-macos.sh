#!/bin/bash
# SnapForge — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything SnapForge needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine SnapForge
...
# Shell overrides (SNAPFORGE_PORT / SNAPFORGE_HOST) take top priority, then .env
# values (APP_PORT / APP_BIND), then built-in defaults.
PORT="${SNAPFORGE_PORT:-${APP_PORT:-7860}}"   # 7860, not 7000 — macOS AirPlay Receiver holds 7000.
HOST="${SNAPFORGE_HOST:-${APP_BIND:-127.0.0.1}}" # Set APP_BIND=0.0.0.0 in .env for LAN/Tailscale access.
PROBE_HOST="$HOST"
...
echo "▶ SnapForge quick start for macOS"
...
if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
    ...
    echo "    SNAPFORGE_PORT=7900 ./start-macos.sh"
    exit 1
fi
...
echo "▶ Preparing SnapForge…"
SNAPFORGE_SKIP_RUN_HINT=1 ./venv/bin/python setup.py
...
if [ "$MACHINE_ARCH" = "arm64" ]; then
    if command -v apfel >/dev/null 2>&1; then
        APFEL_LOG="${TMPDIR:-/tmp}/snapforge-apfel.log"
...
if (exec 3<>"/dev/tcp/127.0.0.1/$CHROMA_PORT") 2>/dev/null; then
    ...
    CHROMA_LOG="${TMPDIR:-/tmp}/snapforge-chromadb.log"
...
# SNAPFORGE_NO_OPEN=1 (e.g. over SSH / headless).
POLLER_PID=""
if [ -z "$SNAPFORGE_NO_OPEN" ] && command -v open >/dev/null 2>&1; then
    (
        for _ in $(seq 1 90); do
            if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
                printf '\n'
                printf '  ┌────────────────────────────────────────────┐\n'
                printf '  │  ✓ SnapForge is ready — opening your browser  │\n'
                printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting SnapForge — it will open in your browser at $URL"

...
try:
    ...
    # SnapForge is single-process by convention, so this is acceptable, but be aware that
...
class _SlowRequestLogMiddleware(_BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        ...
        try:
            ...
            try:
                threshold = float(os.getenv("SNAPFORGE_SLOW_REQUEST_LOG_SECONDS", "0.75") or "0.75")
            except Exception:
                threshold = 0.75
...
if AUTH_ENABLED:
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. SnapForge's own
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-SnapForge-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-SnapForge-Owner") or "").strip()
...
async def _startup_event():
    ...
    _startup_warmups_enabled = str(os.getenv("SNAPFORGE_STARTUP_WARMUPS", "")).lower() in {"1", "true", "yes", "on"}
    if _startup_warmups_enabled:
        ...
        logger.info("Startup warmups disabled (set SNAPFORGE_STARTUP_WARMUPS=1 to enable)")
    ...
    _keepalive_enabled = str(os.getenv("SNAPFORGE_MODEL_KEEPALIVE", "")).lower() in {"1", "true", "yes", "on"}
    ...
    # `SNAPFORGE_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = os.environ.get("SNAPFORGE_INPROCESS_TASKS", "1").strip().lower()
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (SNAPFORGE_INPROCESS_TASKS=0); "

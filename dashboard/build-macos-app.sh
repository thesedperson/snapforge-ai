#!/bin/bash
# Build a downloadable macOS launcher app + .dmg for SnapForge.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/SnapForge.app   — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/SnapForge.dmg   — drag-to-Applications disk image (the downloadable).
#
...
# so rebuild if you move the repo. Override the port with SNAPFORGE_PORT.
set -e

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_NAME="SnapForge"
INSTALL_DIR="$REPO_DIR"
PORT="${SNAPFORGE_PORT:-7860}"
DIST="$REPO_DIR/dist"
...
# ── Icon (best effort) — center-crop docs/snapforge.jpg to a square .icns ──
if [ -f "$REPO_DIR/docs/snapforge.jpg" ] && command -v sips >/dev/null 2>&1; then
  TMPIMG="$(mktemp -d)"
  ...
  sips -c 720 720 "$REPO_DIR/docs/snapforge.jpg" --out "$TMPIMG/sq.png" >/dev/null 2>&1 || cp "$REPO_DIR/docs/snapforge.jpg" "$TMPIMG/sq.png"
  sips -z 512 512 "$TMPIMG/sq.png" --out "$TMPIMG/icon.png" >/dev/null 2>&1
  if sips -s format icns "$TMPIMG/icon.png" --out "$APP/Contents/Resources/snapforge.icns" >/dev/null 2>&1; then
    echo "  icon:        snapforge.icns"
  else
    echo "  icon:        (skipped — conversion failed)"
  fi
  rm -rf "$TMPIMG"
else
  echo "  icon:        (skipped — no docs/snapforge.jpg)"
fi
...
<dict>
    ...
    <key>CFBundleIdentifier</key>      <string>com.snapforge.launcher</string>
    <key>CFBundleVersion</key>         <string>1.0</string>
    <key>CFBundleShortVersionString</key><string>1.0</string>
    <key>CFBundlePackageType</key>     <string>APPL</string>
    <key>CFBundleExecutable</key>      <string>$APP_NAME</string>
    <key>CFBundleIconFile</key>        <string>snapforge</string>
...
#!/bin/bash
# SnapForge.app — start the local server and open the UI in an app window.
...
LOG="$INSTALL_DIR/logs/snapforge-app.log"

notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"SnapForge\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"SnapForge\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "SnapForge isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "SnapForge failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "SnapForge is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag SnapForge to Applications)"

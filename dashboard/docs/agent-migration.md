...
SnapForge should be able to learn from another agent without blindly trusting
...
Markdown notes, or any other agent can have its own adapter, but SnapForge only
...
- **Memory candidates** are short facts or preferences that can be reviewed
  before being saved into SnapForge memory.

This keeps SnapForge' existing memory-review flow intact while giving it better
...
The helper does not write to `data/`, call an LLM, import SnapForge modules, or
...
A future SnapForge importer should treat the manifest as untrusted user-provided
...
Nonstandard folders should be adapter details, not required SnapForge concepts.

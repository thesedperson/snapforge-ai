...
SnapForge keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/snapforge-backup` tool
...
```bash
# Create a snapshot → backups/snapforge-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/snapforge-backup snapshot

# List existing snapshots (most recent first)
./scripts/snapforge-backup list

# Check a tarball's integrity without extracting it
./scripts/snapforge-backup verify backups/snapforge-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/snapforge-backup restore backups/snapforge-backup-20260101-120000.tar.gz --yes
```
...
./scripts/snapforge-backup snapshot --out /mnt/nas/snapforge-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/snapforge-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/snapforge && ./scripts/snapforge-backup snapshot --out "/mnt/nas/snapforge-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `snapforge-backup` therefore does not capture the Docker

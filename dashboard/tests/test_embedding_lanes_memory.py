...
def test_memory_vector_store_writes_both_lanes_and_prefers_custom(monkeypatch):
    ...
    assert fake.collections["snapforge_memories_fastembed"].count() == 1
...
def test_memory_search_merges_fallback_only_results_before_limit():
    custom_collection = FakeCollection("snapforge_memories_custom", metadata={"embedding_lane": "custom"})
    fast_collection = FakeCollection("snapforge_memories_fastembed", metadata={"embedding_lane": "fastembed"})
    ...
    custom_lane = EmbeddingLane(
        ...
        collection_name="snapforge_memories_custom",
        model="nomic",
    ...
    fast_lane = EmbeddingLane(
        ...
        collection_name="snapforge_memories_fastembed",
        model="mini",
        url="local://fastembed",
        dimension=384,
        fingerprint="fast",
...
def test_memory_rebuild_does_not_reimport_legacy_collection(monkeypatch):
    fake = FakeChroma()
    legacy = fake.get_or_create_collection("snapforge_memories", metadata={"hnsw:space": "cosine"})
    legacy.add(
        ids=["stale-memory"],
        embeddings=[[0.0] * 384],
    ...
    inactive_custom = fake.get_or_create_collection("snapforge_memories_custom", metadata={"embedding_lane": "custom"})
    inactive_custom.add(
        ids=["stale-custom"],
        embeddings=[[0.0] * 768],
    ...
    assert "snapforge_memories" not in fake.collections
    assert "snapforge_memories_custom" not in fake.collections
    assert fake.collections["snapforge_memories_fastembed"].count() == 1
    assert fake.collections["snapforge_memories_fastembed"].get()["ids"] == ["current-memory"]


def test_memory_remove_deletes_inactive_lane_collection(monkeypatch):
    fake = FakeChroma()
    custom_collection = fake.get_or_create_collection("snapforge_memories_custom", metadata={"embedding_lane": "custom"})
    fast_collection = fake.get_or_create_collection("snapforge_memories_fastembed", metadata={"embedding_lane": "fastembed"})
    custom_collection.add(
        ids=["mem-1"],
        embeddings=[[0.0] * 768],
...
def test_memory_rebuild_continues_when_custom_lane_fails(monkeypatch):
    ...
    assert fake.collections["snapforge_memories_custom"].count() == 0
    assert fake.collections["snapforge_memories_fastembed"].count() == 1
    assert fake.collections["snapforge_memories_fastembed"].get()["ids"] == ["current-memory"]

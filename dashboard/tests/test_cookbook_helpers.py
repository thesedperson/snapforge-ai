...
def test_vllm_preflight_reports_cli_and_version():
    ...
    assert 'SNAPFORGE_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"' in script
    assert 'echo "[snapforge] vLLM CLI: $SNAPFORGE_VLLM_BIN"' in script
    assert '"$SNAPFORGE_VLLM_BIN" --version' in script
    assert 'SNAPFORGE_PREFLIGHT_EXIT=127' in script
...
def test_serve_preflight_failure_keeps_tmux_pane_visible():
    ...
    assert 'SNAPFORGE_PREFLIGHT_EXIT=127' in script
    assert 'echo "=== Process exited with code $SNAPFORGE_PREFLIGHT_EXIT ==="' in script
...
def test_serve_runner_preserves_command_exit_code():
    ...
    assert "SNAPFORGE_CMD_EXIT=$?" in script
    assert 'echo "=== Process exited with code $SNAPFORGE_CMD_EXIT ==="' in script
...
def test_pip_serve_runner_emits_download_ok_before_exit_marker():
    ...
    assert 'exit "$SNAPFORGE_CMD_EXIT"' in script
...
def test_llama_cpp_linux_bootstrap_checks_cudart_before_cuda_build():
    ...
    assert '_snapforge_has_cudart' in script
    ...
    assert script.index('_snapforge_has_cudart') < script.index('DGGML_CUDA=ON')
...
def test_cached_model_scan_uses_ollama_api_before_cli_and_windows_opt_in():
    ...
    assert "SNAPFORGE_ALLOW_OLLAMA_CLI_SCAN" in script
...
def test_cached_model_scan_does_not_launch_ollama_cli_on_windows(tmp_path):
    ...
    env.pop("SNAPFORGE_ALLOW_OLLAMA_CLI_SCAN", None)
    proc = subprocess.run(
        [sys.executable, str(scan_py)],
        check=True,
        capture_output=True,

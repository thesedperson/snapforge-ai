...
def test_legacy_collection_backfills_fastembed_lane(monkeypatch):
    ...
    assert fake.collections["snapforge_memories"].count() == 1
...
def test_legacy_collection_backfills_custom_only_lane(monkeypatch):
    ...
    assert store.count() == 1
    assert "snapforge_memories_fastembed" not in fake.collections
    assert fake.collections["snapforge_memories_custom"].count() == 1
    assert len(fake.collections["snapforge_memories_custom"].rows["legacy-memory"]["embedding"]) == 768
...
def test_legacy_migration_resumes_partial_lane_backfill(monkeypatch):
    ...
    partial = fake.get_or_create_collection("snapforge_memories_fastembed", metadata={"embedding_lane": "fastembed"})
    partial.add(
        ids=["legacy-1"],
        embeddings=[[0.0] * 384],
    ...
    assert set(fake.collections["snapforge_memories_fastembed"].get()["ids"]) == {"legacy-1", "legacy-2"}

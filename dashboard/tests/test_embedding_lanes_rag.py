...
def test_vector_rag_writes_both_lanes_and_falls_back_to_fastembed(monkeypatch):
    ...
    assert "snapforge_rag_custom" not in fake.collections
...
def test_vector_rag_batch_index_continues_when_custom_lane_fails(monkeypatch, tmp_path):
    ...
    assert fake.collections["snapforge_rag_custom"].count() == 0
    assert fake.collections["snapforge_rag_fastembed"].count() == 1
...
def test_vector_rag_batch_index_reports_failure_when_all_lanes_fail(monkeypatch, tmp_path):
    ...
    assert fake.collections["snapforge_rag_custom"].count() == 0
    assert fake.collections["snapforge_rag_fastembed"].count() == 0


def test_rag_rebuild_does_not_reimport_legacy_collection(monkeypatch, tmp_path):
    fake = FakeChroma()
    legacy = fake.get_or_create_collection("snapforge_rag", metadata={"hnsw:space": "cosine"})
    legacy.add(
        ids=["stale-doc"],
        embeddings=[[0.0] * 384],
    ...
    inactive_custom = fake.get_or_create_collection("snapforge_rag_custom", metadata={"embedding_lane": "custom"})
    inactive_custom.add(
    ...
    assert "snapforge_rag" not in fake.collections
    assert "snapforge_rag_custom" not in fake.collections
    assert fake.collections["snapforge_rag_fastembed"].count() == 0
...
def test_rag_remove_directory_deletes_inactive_lane_collection(monkeypatch, tmp_path):
    fake = FakeChroma()
    legacy_collection = fake.get_or_create_collection("snapforge_rag", metadata={"hnsw:space": "cosine"})
    custom_collection = fake.get_or_create_collection("snapforge_rag_custom", metadata={"embedding_lane": "custom"})
    fast_collection = fake.get_or_create_collection("snapforge_rag_fastembed", metadata={"embedding_lane": "fastembed"})
    ...
    fast_lane = EmbeddingLane(
        ...
        collection_name="snapforge_rag_fastembed",
        model="mini",
        url="local://fastembed",
        dimension=384,
        fingerprint="fast",
...
def test_vector_rag_uses_keyword_fallback_when_all_lanes_query_fail():
    collection = FakeCollection("snapforge_rag_fastembed", metadata={"embedding_lane": "fastembed"})
    collection.add(
        ids=["doc-1"],
        embeddings=[[0.0] * 384],

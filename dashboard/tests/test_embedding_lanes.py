...
def test_build_embedding_lanes_keeps_custom_and_fastembed_dimensions_separate(monkeypatch):
    ...
    built = build_embedding_lanes("snapforge_memories")

    assert [lane.name for lane in built] == [LANE_CUSTOM, LANE_FASTEMBED]
    assert built[0].collection_name == "snapforge_memories_custom"
    assert built[0].dimension == 768
    assert built[1].collection_name == "snapforge_memories_fastembed"
...
def test_build_embedding_lanes_recreates_only_custom_when_fingerprint_changes(monkeypatch):
    ...
    built = build_embedding_lanes("snapforge_rag")

    assert "snapforge_rag_custom" in fake.deleted
    assert fake.collections["snapforge_rag_custom"].count() == 1
    assert len(fake.collections["snapforge_rag_custom"].rows["old"]["embedding"]) == 1024
    assert fake.collections["snapforge_rag_fastembed"].count() == 1
...
def test_lane_reset_reembeds_existing_documents_on_fingerprint_change(monkeypatch):
    ...
    assert "snapforge_memories_custom" in fake.deleted
    rebuilt = fake.collections["snapforge_memories_custom"]
...
def test_lane_reset_keeps_existing_collection_when_reembed_fails(monkeypatch):
    ...
    assert "snapforge_memories_custom" not in fake.deleted
    assert fake.collections["snapforge_memories_custom"].count() == 1
    assert len(fake.collections["snapforge_memories_custom"].rows["existing-memory"]["embedding"]) == 384
...
def test_lane_reset_keeps_existing_collection_when_preserve_read_fails(monkeypatch):
    ...
    assert built == []
    assert "snapforge_memories_custom" not in fake.deleted
    assert "snapforge_memories_custom" in fake.collections
...
def test_lane_reset_restores_existing_collection_when_rewrite_fails(monkeypatch):
    ...
    fake.fail_next_add_for["snapforge_memories_custom"] = 1
    patch_chroma(monkeypatch, fake)
    ...
    assert built == []
    restored = fake.collections["snapforge_memories_custom"]
...
def test_build_embedding_lanes_uses_fastembed_when_custom_unavailable(monkeypatch):
    ...
    built = build_embedding_lanes("snapforge_tool_index")

    assert [lane.name for lane in built] == [LANE_FASTEMBED]
    assert built[0].collection_name == "snapforge_tool_index_fastembed"

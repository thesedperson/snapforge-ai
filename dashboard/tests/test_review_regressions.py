...
async def test_legacy_mcp_tools_decode_inline_json_args(monkeypatch):
    ...
    cases = {
        "web_search": ('{"query": "snapforge pr 3681"}', {"query": "snapforge pr 3681"}),
...
async def test_plan_mode_blocks_mutating_email_aliases_without_mcp_inventory(monkeypatch):
    ...
    assert result["exit_code"] == 0
    assert mcp.calls == [
        ("mcp__email__search_emails", {"query": "x", "_snapforge_owner": "admin-user"}),
    ]
    ...
    assert result["exit_code"] == 0
    assert mcp.calls == [
        ("mcp__email__scan_email_unsubscribes", {"limit": 1, "_snapforge_owner": "admin-user"}),
    ]
...
async def test_bare_email_dispatch_empty_content_calls_with_empty_args(monkeypatch):
    ...
    assert result["exit_code"] == 0
    assert mcp.calls == [
        ("mcp__email__list_email_accounts", {"_snapforge_owner": "admin-user"}),
    ]
...
async def test_email_mcp_dispatch_includes_hidden_owner(monkeypatch):
    ...
    assert result["exit_code"] == 0
    assert fake.calls == [
        ("mcp__email__list_emails", {"folder": "INBOX", "_snapforge_owner": "alice"}),
    ]

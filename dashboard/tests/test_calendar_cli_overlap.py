"""Regression: `snapforge-calendar list` must select events that OVERLAP the
...
`snapforge-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "snapforge-calendar"
    loader = importlib.machinery.SourceFileLoader("snapforge_calendar_cli", str(path))

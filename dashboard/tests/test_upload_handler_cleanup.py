...
def test_reference_discovery_covers_all_durable_upload_stores(
    ...
    try:
        ...
        db.add(Note(
            ...
            color=f"snapforge://attachment/{note_color_id}",
        ))
        db.add(CalendarCal(
            id="calendar-1",
            owner="alice",
        ...
        db.add(CalendarEvent(
            ...
            description=f"Notes: snapforge://attachment/{event_description_id}",
...
def test_write_reservation_extracts_only_explicit_internal_references():
    ...
    assert extract_internal_upload_ids({
        ...
        "nested": [f"snapforge://attachment/{upload_id}"],
    }) == {upload_id}
    assert extract_internal_upload_ids(
    ...
    assert extract_internal_upload_ids(
        f"Attachment: snapforge://attachment/{extensionless_id}: ready"
    ) == {extensionless_id}
...
def test_reservation_never_uses_admin_override(tmp_path):
    ...
    assert reserve_upload_references(
        handler,
        "alice",
        f"snapforge://attachment/{upload_id}",
    ) is None
    assert reserve_upload_references(
        handler,
        "admin",
    ...
    with pytest.raises(HTTPException) as calendar_error:
        asyncio.run(create_event(
            ...
            EventCreate(
                ...
                color=f"snapforge://attachment/{upload_id}",
            ),
        ))

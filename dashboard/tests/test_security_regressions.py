...
def test_ollama_cookbook_runner_does_not_force_public_bind():
    ...
    assert 'OLLAMA_HOST="0.0.0.0:${SNAPFORGE_OLLAMA_PORT}" ollama serve' not in route
    assert 'OLLAMA_HOST="${SNAPFORGE_OLLAMA_HOST}:${SNAPFORGE_OLLAMA_PORT}" ollama serve' in route
...
def test_inprocess_pollers_gate(monkeypatch):
    """The SNAPFORGE_INPROCESS_POLLERS env var must let operators kill
    the asyncio pollers when cron / systemd is driving the one-shot
    `snapforge-mail poll-*` CLI subcommands instead. Two pollers racing
    ...
    monkeypatch.delenv("SNAPFORGE_INPROCESS_POLLERS", raising=False)
    assert _inprocess_pollers_enabled() is True

    # Any of the off-values disables.
    for off in ("0", "false", "no", "off", "FALSE", "Off"):
        monkeypatch.setenv("SNAPFORGE_INPROCESS_POLLERS", off)
    ...
    for on in ("1", "true", "yes", "anything-truthy"):
        monkeypatch.setenv("SNAPFORGE_INPROCESS_POLLERS", on)
...
def _load_search_content_for_test(monkeypatch, name="services.search.content_under_test"):
    ...
    cache.CONTENT_CACHE_DIR = Path("/tmp/snapforge-test-content-cache")
...
def test_google_mcp_oauth_uses_configured_redirect_base(monkeypatch):
    monkeypatch.setenv("OAUTH_REDIRECT_BASE_URL", "https://snapforge.example/app/")
    ...
    assert (
        ...
        == "https://snapforge.example/app/api/mcp/oauth/callback"
    )

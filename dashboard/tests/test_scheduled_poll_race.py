...
in-process 30s poller and the `snapforge-mail poll-scheduled` CLI, which the
project's own docstrings warn can race on the same SQLite when
SNAPFORGE_INPROCESS_POLLERS is left enabled alongside an external cron/systemd

...
def _load_setup_module():
    spec = importlib.util.spec_from_file_location("snapforge_setup_under_test", Path("setup.py"))
...
def test_create_default_admin_normalizes_env_username(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SNAPFORGE_ADMIN_USER", " AdminUser ")
    monkeypatch.setenv("SNAPFORGE_ADMIN_PASSWORD", "temporary-password")
...
def test_main_loads_admin_password_from_env_file(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("SNAPFORGE_ADMIN_USER", raising=False)
    monkeypatch.delenv("SNAPFORGE_ADMIN_PASSWORD", raising=False)
    (tmp_path / ".env").write_text(
        "SNAPFORGE_ADMIN_USER=presetuser\nSNAPFORGE_ADMIN_PASSWORD=fromenvfile12345\n",
        encoding="utf-8-sig",
    )
    ...
    monkeypatch.setenv("SNAPFORGE_SKIP_ADMIN_PROMPT", "1")
    ...
    try:
        ...
        # don't inherit them.
        os.environ.pop("SNAPFORGE_ADMIN_USER", None)
        os.environ.pop("SNAPFORGE_ADMIN_PASSWORD", None)

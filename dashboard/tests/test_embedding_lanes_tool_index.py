...
def test_tool_index_indexes_and_retrieves_from_available_lanes(monkeypatch):
    ...
    assert fake.collections["snapforge_tool_index_custom"].count() > 0
    assert fake.collections["snapforge_tool_index_fastembed"].count() > 0
...
def test_tool_index_builtin_indexing_fails_when_all_lanes_fail():
    custom_lane = EmbeddingLane(
        ...
        collection=FakeCollection("snapforge_tool_index_custom", metadata={"embedding_lane": "custom"}),
        collection_name="snapforge_tool_index_custom",
        model="nomic",
    ...
    fast_lane = EmbeddingLane(
        ...
        collection=FakeCollection("snapforge_tool_index_fastembed", metadata={"embedding_lane": "fastembed"}),
        collection_name="snapforge_tool_index_fastembed",
        model="mini",
        url="local://fastembed",
        dimension=384,
        fingerprint="fast",
...
def test_tool_index_retrieval_continues_when_custom_lane_query_fails():
    custom_collection = FakeCollection("snapforge_tool_index_custom", metadata={"embedding_lane": "custom"})
    fast_collection = FakeCollection("snapforge_tool_index_fastembed", metadata={"embedding_lane": "fastembed"})
    fast_collection.add(
        ids=["builtin_bash"],
        embeddings=[[0.0] * 384],
        documents=["Tool: bash\nRun shell commands"],

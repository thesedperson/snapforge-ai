...
base compose with only the matching overlay merged into the ``snapforge``
...
def _merge_overlay_into_base(base: dict, overlay: dict) -> dict:
    """Build the expected standalone config: base + overlay on snapforge only."""
...
# --- Non-snapforge services and volumes untouched ---------------------------


@pytest.mark.parametrize("standalone_path", [NVIDIA_STANDALONE, AMD_STANDALONE])
def test_non_snapforge_services_match_base(base, standalone_path):
...
# --- snapforge = base service + only the overlay additions ------------------


def test_nvidia_snapforge_adds_only_overlay(base):
    standalone = _load(NVIDIA_STANDALONE)
...
def test_amd_snapforge_adds_only_overlay(base):
    standalone = _load(AMD_STANDALONE)
    svc = standalone["services"][SERVICE]
    base_svc = base["services"][SERVICE]
...
def test_base_has_no_host_docker_access(base):
    ...
    assert "SNAPFORGE_ENABLE_HOST_DOCKER=true" not in service["environment"]
...
def test_base_plus_host_docker_overlay_has_explicit_access(base):
    ...
    assert "SNAPFORGE_ENABLE_HOST_DOCKER=true" in service["environment"]

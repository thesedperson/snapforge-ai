...
a bare basename in SNAPFORGE_MCP_ALLOWED_COMMANDS; code-exec flags are rejected
...
def _env(monkeypatch):
    ...
    monkeypatch.setenv("SNAPFORGE_MCP_ALLOWED_COMMANDS", "mcp-server-demo,python3")
    db = _TS()
    try:
...
def test_denied_command_rejected_even_when_operator_allowlists_it():
    # python3 is in SNAPFORGE_MCP_ALLOWED_COMMANDS for this test; hard-deny wins.
...
def test_alias_runtime_denied_even_if_operator_allowlists_it(monkeypatch):
    ...
    monkeypatch.setenv("SNAPFORGE_MCP_ALLOWED_COMMANDS", "python3.11,node18,java,bunx")

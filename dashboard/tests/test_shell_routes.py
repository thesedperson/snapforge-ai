...
class TestRunningInContainer:
    """Detect whether the SnapForge process itself runs inside a container."""
...
class TestHostDockerAccess:
    def test_opt_in_without_socket_is_disabled(self, monkeypatch, tmp_path):
        monkeypatch.setenv("SNAPFORGE_ENABLE_HOST_DOCKER", "true")
    ...
    def test_socket_without_explicit_opt_in_is_disabled(
        ...
        with socket.socket(socket.AF_UNIX) as unix_socket:
            ...
            if flag is None:
                monkeypatch.delenv("SNAPFORGE_ENABLE_HOST_DOCKER", raising=False)
            else:
                monkeypatch.setenv("SNAPFORGE_ENABLE_HOST_DOCKER", flag)
...
class TestPackageProbeStatus:
    ...
    def test_vllm_cli_without_dist_is_external_for_update(self):
        ...
        assert "outside SnapForge" in status.note
    ...
    def test_apfel_does_not_use_generic_outside_snapforge_note(self):
        status = _package_pip_update_status(
        ...
        assert status.available is False
        assert "Update this system dependency outside SnapForge." not in status.note

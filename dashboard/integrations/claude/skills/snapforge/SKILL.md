---
name: snapforge
description: Use when the user asks Claude Code to read or write SnapForge data (todos, email, calendar, memory, documents) or to launch/monitor/stop a Cookbook model-serve task through the scoped Claude Agent API. Requires SNAPFORGE_URL and SNAPFORGE_API_TOKEN.
---

# SnapForge

Use this skill when a user asks to interact with SnapForge from Claude Code.

## Configuration

Expect these environment variables:

- `SNAPFORGE_URL`: Base URL for the user's SnapForge instance, for example `http://127.0.0.1:7000`.
- `SNAPFORGE_API_TOKEN`: Scoped API token created in SnapForge Settings > Integrations > Add Integration > Claude Agent.

If either value is missing, do not guess credentials. Tell the user to create a Claude Agent token in SnapForge Settings and expose both values to the terminal session.
...
- **Calendar event ("meeting at 3pm", "dentist Tuesday 10am")** → calendar event. Use for scheduled time blocks, meetings, appointments, recurring schedules. These show up on the calendar grid; reminders for them are configured separately in SnapForge settings.
...
- All SnapForge data access MUST go through the scoped HTTP API under `/api/codex/*` (the canonical scope-gated agent API, shared by all agent integrations).
...
- Do not use SSH, Docker, direct Python imports, SQLite queries, MCP internals, browser cookies, or local files to read/write SnapForge user data.
...
```bash
python3 ~/.claude/skills/snapforge/scripts/snapforge_api.py capabilities
python3 ~/.claude/skills/snapforge/scripts/snapforge_api.py todos list
python3 ~/.claude/skills/snapforge/scripts/snapforge_api.py todos add "Follow up"
```
...
```bash
python3 ~/.claude/skills/snapforge/scripts/snapforge_api.py POST /api/codex/todos '{"action":"add","title":"Call dentist","due_date":"tomorrow at 5pm"}'
```
...
```bash
python3 ~/.claude/skills/snapforge/scripts/snapforge_api.py emails list 5
python3 ~/.claude/skills/snapforge/scripts/snapforge_api.py emails read UID
```
...
```bash
python3 ~/.claude/skills/snapforge/scripts/snapforge_api.py GET /api/codex/memory
python3 ~/.claude/skills/snapforge/scripts/snapforge_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
- Prefer `POST /api/codex/emails/draft-document` for agent-written email replies. It creates an editable SnapForge Document with `language: "email"` and does not touch IMAP/send.
...
python3 ~/.claude/skills/snapforge/scripts/snapforge_api.py cookbook tasks

# Tail the failing one (sessionId from `cookbook tasks`)
python3 ~/.claude/skills/snapforge/scripts/snapforge_api.py cookbook output serve-abc12345 400

# Stop the previous attempt before you try a new flag set
python3 ~/.claude/skills/snapforge/scripts/snapforge_api.py cookbook stop serve-abc12345

# Relaunch with new flags. cmd MUST begin with one of the allowlisted binaries.
python3 ~/.claude/skills/snapforge/scripts/snapforge_api.py cookbook serve \
...
If you are about to reach the SnapForge host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass SnapForge Settings and token scopes. Ask the user to enable the relevant Claude Agent tool toggle instead.

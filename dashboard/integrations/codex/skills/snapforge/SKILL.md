---
name: snapforge
description: Use when the user asks Codex to read or write SnapForge data (todos, email, calendar, memory, documents) or to launch/monitor/stop a Cookbook model-serve task through the scoped Codex Agent API. Requires SNAPFORGE_URL and SNAPFORGE_API_TOKEN.
---

# SnapForge

Use this skill when a user asks to interact with SnapForge from Codex.
...
- `SNAPFORGE_API_TOKEN`: Scoped API token created in SnapForge Settings > Integrations > Add Integration > Codex Agent.

If either value is missing, do not guess credentials. Tell the user to create a Codex Agent token in SnapForge Settings and expose both values to the terminal session.
...
- All SnapForge data access MUST go through the scoped HTTP API under `/api/codex/*`.
...
```bash
python3 integrations/codex/scripts/snapforge_api.py capabilities
python3 integrations/codex/scripts/snapforge_api.py todos list
python3 integrations/codex/scripts/snapforge_api.py todos add "Follow up"
```
...
```bash
python3 integrations/codex/scripts/snapforge_api.py POST /api/codex/todos '{"action":"add","title":"Call dentist","due_date":"tomorrow at 5pm"}'
```
...
```bash
python3 integrations/codex/scripts/snapforge_api.py emails list 5
python3 integrations/codex/scripts/snapforge_api.py emails read UID
```
...
```bash
python3 integrations/codex/scripts/snapforge_api.py GET /api/codex/memory
python3 integrations/codex/scripts/snapforge_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
- Prefer `POST /api/codex/emails/draft-document` for Codex-written email replies. It creates an editable SnapForge Document with `language: "email"` and does not touch IMAP/send.
...
```bash
python3 ~/plugins/snapforge/scripts/snapforge_api.py cookbook tasks
python3 ~/plugins/snapforge/scripts/snapforge_api.py cookbook output serve-abc12345 400
python3 ~/plugins/snapforge/scripts/snapforge_api.py cookbook stop serve-abc12345
python3 ~/plugins/snapforge/scripts/snapforge_api.py cookbook serve \
...
If you are about to reach the SnapForge host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass SnapForge Settings and token scopes. Ask the user to enable the relevant Codex Agent tool toggle instead.

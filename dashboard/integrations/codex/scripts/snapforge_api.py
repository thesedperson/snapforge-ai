#!/usr/bin/env python3
"""Small SnapForge scoped API helper for Codex terminal sessions."""
...
def _usage() -> int:
    ...
    print("  snapforge_api.py capabilities", file=sys.stderr)
    print("  snapforge_api.py todos list", file=sys.stderr)
    print("  snapforge_api.py todos add TITLE", file=sys.stderr)
    print("  snapforge_api.py emails list [limit]", file=sys.stderr)
    print("  snapforge_api.py emails read UID", file=sys.stderr)
    print("  snapforge_api.py emails draft-doc JSON_PAYLOAD", file=sys.stderr)
    print("  snapforge_api.py documents list [limit]", file=sys.stderr)
    print("  snapforge_api.py documents read DOC_ID", file=sys.stderr)
    print("  snapforge_api.py documents create JSON_PAYLOAD", file=sys.stderr)
    print("  snapforge_api.py documents delete DOC_ID", file=sys.stderr)
    print("  snapforge_api.py cookbook tasks", file=sys.stderr)
    print("  snapforge_api.py cookbook servers", file=sys.stderr)
    print("  snapforge_api.py cookbook cached [HOST]", file=sys.stderr)
    print("  snapforge_api.py cookbook presets", file=sys.stderr)
    print("  snapforge_api.py cookbook output SESSION_ID [tail]", file=sys.stderr)
    print("  snapforge_api.py cookbook serve REPO_ID 'CMD' [REMOTE_HOST]", file=sys.stderr)
    print("  snapforge_api.py cookbook preset NAME", file=sys.stderr)
    print("  snapforge_api.py cookbook adopt SESSION_ID MODEL [HOST] [PORT]", file=sys.stderr)
    print("  snapforge_api.py cookbook stop SESSION_ID", file=sys.stderr)
    print("  snapforge_api.py METHOD /api/codex/path [json-body]", file=sys.stderr)
    return 2


def _config() -> tuple[str, str] | None:
    base_url = os.environ.get("SNAPFORGE_URL", "").strip().rstrip("/")
    token = os.environ.get("SNAPFORGE_API_TOKEN", "").strip()
    missing = []
    if not base_url:
        missing.append("SNAPFORGE_URL")
    if not token:
        missing.append("SNAPFORGE_API_TOKEN")
    if missing:
        print(f"missing {', '.join(missing)}; create a Codex Agent token in SnapForge Settings", file=sys.stderr)
        return None
...
def main() -> int:
    ...
    if not path.startswith("/api/codex/"):
        print("refusing non-/api/codex path; use scoped SnapForge integration endpoints only", file=sys.stderr)
        return 2

...
shape SnapForge already uses for local image endpoints, then delegates to the
...
app = FastAPI(title="SnapForge MLX Image Server")
...
def _unsupported_swift_mlx_runtime(model: str) -> HTTPException:
    if _is_ddcolor(model):
        return HTTPException(
            503,
            "DDColor MLX models require an SnapForge-compatible mlx-ddcolor-swift bridge. "
            "Build/install a bridge binary named snapforge-mlx-colorize or mlx-ddcolor-serve "
            "on the Apple Silicon host PATH. Upstream currently ships Swift libraries and "
            "smoke executables, not a stable colorize CLI.",
        )
    return HTTPException(
        503,
        "LaMa / MI-GAN MLX inpainting models require an SnapForge-compatible mlx-lama-swift bridge. "
        "Build/install a bridge binary named snapforge-mlx-inpaint or mlx-lama-serve "
        "on the Apple Silicon host PATH. Upstream currently ships Swift libraries and "
...
def _run_ddcolor_bridge(model: str, image_raw: bytes, out_path: Path) -> None:
    bridge = _resolve_bridge(["snapforge-mlx-colorize", "mlx-ddcolor-serve"])
    if not bridge:
        raise _unsupported_swift_mlx_runtime(model)
    with tempfile.TemporaryDirectory(prefix="snapforge-ddcolor-") as td:
...
def _run_inpaint_bridge(model: str, image_raw: bytes, mask_raw: bytes | None, out_path: Path) -> None:
    if not mask_raw:
        raise HTTPException(
            422,
            "LaMa / MI-GAN inpainting requires an image mask. Use the editor inpaint/object-removal tool so SnapForge can send the mask.",
        )
    bridge = _resolve_bridge(["snapforge-mlx-inpaint", "mlx-lama-serve"])
    if not bridge:
        raise _unsupported_swift_mlx_runtime(model)
    with tempfile.TemporaryDirectory(prefix="snapforge-mlx-inpaint-") as td:
        inp = Path(td) / "input.png"
...
def _generate_boogu(model: str, prompt: str, out_path: Path, width: int, height: int, steps: int) -> None:
    ...
    model_path = _snapshot_path(model)
    vlm_model = (_args.vlm_model or os.environ.get("SNAPFORGE_MLX_IMAGE_VLM_MODEL") or "").strip()
    if not vlm_model:
        raise HTTPException(
            422,
            "This MLX image pipeline requires a companion vision-language model. "
            "Relaunch with --vlm-model <repo_or_path> or set SNAPFORGE_MLX_IMAGE_VLM_MODEL.",
        )
    try:
...
def generate(req: ImageRequest):
    ...
    for _ in range(count):
        with tempfile.TemporaryDirectory(prefix="snapforge-mlx-image-") as td:
...
async def edit_image(
    ...
    if _is_lama_inpaint(active_model) or _is_ddcolor(active_model):
        ...
        for _ in range(count):
            with tempfile.TemporaryDirectory(prefix="snapforge-mlx-edit-") as td:
                out_path = Path(td) / "image.png"
...
def harmonize_image(req: HarmonizeRequest):
    ...
    if _is_lama_inpaint(active_model) or _is_ddcolor(active_model):
        ...
        with tempfile.TemporaryDirectory(prefix="snapforge-mlx-harmonize-") as td:
            out_path = Path(td) / "image.png"
            if _is_ddcolor(active_model):
                _run_ddcolor_bridge(active_model, image_raw, out_path)
            else:

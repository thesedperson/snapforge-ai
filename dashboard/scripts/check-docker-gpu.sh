...
# The SnapForge app never calls this script automatically.
#
# USAGE
...
After a successful setup, start SnapForge:
...
_enable_nvidia_overlay() {
    ...
    if echo "${_current_cf}" | grep -qF "${_overlay_fragment}"; then
        ...
        echo
        _info "Start or restart SnapForge to apply:"
    ...
    echo
    _info "Start or restart SnapForge with the NVIDIA overlay:"
    _info "  docker compose up -d --build"
    echo
...
_mode_check() {
    echo "=== SnapForge Docker GPU diagnostic ==="
    echo
    _check_nvidia_smi
    ...
    if [ "${OPT_ENABLE_OVERLAY}" -eq 1 ]; then
        if [ "${_GPU_PASSTHROUGH_OK}" -eq 0 ]; then
            ...
            # Writing COMPOSE_FILE before passthrough works causes SnapForge to fail

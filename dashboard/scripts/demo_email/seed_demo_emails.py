...
This populates the `demo@snapforge.local` Dovecot account (which has NO mbsync
...
USER = os.getenv("DEMO_IMAP_USER", "demo@snapforge.local")
...
LOOKUP_MSGID = "<demo-lookup-deepseek@snapforge.local>"
...
def _msg(*, frm, to=None, subject, text, html=None, days_ago=0, hours_ago=0,
    ...
    m["Message-ID"] = msg_id or make_msgid(domain="snapforge.local")
    if in_reply_to:
...
def build_dataset() -> list[dict]:
    ...
    add("INBOX", "", _msg(
        ...
        text=("Hey,\n\nSaw your work on the SnapForge stack — seriously impressive. "
        ...
        html=("<p>Hey,</p><p>Saw your work on the <b>SnapForge</b> stack — seriously "
    ...
    add("INBOX", "", _msg(
        frm="Greg <greg@snapforge-demo.example>",
    ...
    orig_id = make_msgid(domain="snapforge.local")
    add("Sent", "(\\Seen)", _msg(
    ...
    add("INBOX", "(\\Flagged)", _msg(
        frm="Ops Bot <ops@snapforge-demo.example>",
        subject="[URGENT] prod is on fire 🔥 — snapforge-ui 502s",
        days_ago=0, hours_ago=1,
        text=("PAGE: snapforge-ui is returning 502s on the /api/chat endpoint.\n"
...
def _seed_cache() -> None:
    ...
    reply = (
        ...
        "— drafted for you by your SnapForge assistant"
    )
    ...
    try:
        ...
        con.execute(
            ...
            (LOOKUP_MSGID, "", "INBOX", "quick q for the slide — DeepSeek-V3 param count?",
             "greg@snapforge-demo.example", summary, "demo", now))
        con.commit()
...
def _wipe(conn: imaplib.IMAP4) -> int:
    ...
    """
    safe_target = USER.endswith("@snapforge.local") or HOST in ("localhost", "127.0.0.1", "::1")
...
def main() -> int:
    ...
    try:
        ...
        print(f"seeded {len(items)} demo message(s) into {USER} "
              f"(INBOX + Sent). Switch to the 'Demo' account in SnapForge to view.")
        return 0
    finally:
        try:
            conn.logout()

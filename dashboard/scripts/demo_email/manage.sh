...
# a switchable 'Demo' account in SnapForge + fake seed mail.
#
...
DOCKER_DIR="${SNAPFORGE_DEMO_MAIL_DIR:-$HOME/docker/snappymail}"
USERS_FILE="$DOCKER_DIR/dovecot/conf/users"
DEMO_USER="demo@snapforge.local"
DEMO_PASS="demodemo"
...
case "${1:-}" in
  setup)
    ...
    echo
    echo "Done. In SnapForge, switch to the 'Demo' account to show off the inbox."
    ;;
  reseed)
    "$PY" "$HERE/seed_demo_emails.py" --reset
    ;;

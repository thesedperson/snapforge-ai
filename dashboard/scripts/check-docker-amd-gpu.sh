...
# see /dev/kfd and /dev/dri. The SnapForge slim image does not include ROCm tools
...
RENDER_GID=""
VIDEO_GID=""
TEST_IMAGE="${SNAPFORGE_AMD_TEST_IMAGE:-alpine:3.20}"
...
Environment:
  SNAPFORGE_AMD_TEST_IMAGE   Docker image for the passthrough smoke
                            (default: alpine:3.20)
USAGE
}
...
_print_next_steps() {
    ...
    echo
    echo "After restarting SnapForge, verify the slim app container sees devices:"
    echo "  docker compose exec snapforge sh -lc 'test -e /dev/kfd && test -d /dev/dri && ls -l /dev/kfd /dev/dri/renderD*'"
    echo
    echo "Note: rocm-smi/rocminfo are not expected inside the slim SnapForge image."
...
echo "=== SnapForge AMD Docker GPU diagnostic ==="
echo
_check_host_devices
_check_groups
_check_host_rocm

...
class WebhookManager:
    ...
    async def deliver_test(self, webhook_id: str, url: str, encrypted_secret: Optional[str]):
        ...
        await self._deliver(webhook_id, url, decrypted, "webhook.test", {"message": "Test ping from SnapForge"})
    ...
    async def _deliver(self, webhook_id: str, url: str, secret: Optional[str], event: str, payload: dict):
        ...
        headers = {
            ...
            "User-Agent": "SnapForge-Webhook/1.0",
        }
        if secret:
            sig = hmac.new(secret.encode(), body.encode(), hashlib.sha256).hexdigest()
            headers["X-SnapForge-Signature"] = sig

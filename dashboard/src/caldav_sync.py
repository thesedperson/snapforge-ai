...
def _private_caldav_allowed() -> bool:
    return os.environ.get("SNAPFORGE_ALLOW_PRIVATE_CALDAV", "0").lower() in {"1", "true", "yes"}
...
def _validate_caldav_address(addr: ipaddress._BaseAddress) -> None:
    ...
    if addr.is_private and not _private_caldav_allowed():
        raise ValueError("Private CalDAV IPs require SNAPFORGE_ALLOW_PRIVATE_CALDAV=1")

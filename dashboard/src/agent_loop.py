...
Streaming agent loop for snapforge-ui.
...
- Tool toggles like "turn off shell/search/research" use `ui_control toggle <name> <on|off>`, not memory.""",
    "sessions": """\
## Chat/session rules
- SnapForge chats are sessions. Use `list_sessions`/`manage_session`; do not shell out looking for chat files.
...
```
GENERIC LOOPBACK to allowed SnapForge internal endpoints. Use this whenever the user wants something the UI can do but there's NO named tool for it. Many UI buttons hit /api/* endpoints — you can hit allowed ones. Auth is handled automatically.
...
def _local_computer_rules() -> str:
    return (
        "\n\n## SnapForge Terminus local-machine mode\n"
...
def _minimal_saved_memory_message(messages: List[Dict]) -> Optional[Dict]:
    ...
    if not facts:
        return None
    logger.info("[agent-intent] snapforge doc minimal memory facts=%s", len(facts))
    return {
        "role": "user",
        "content": (
            "Saved user memory facts from SnapForge Brain. These are the same "
...
def _minimal_recent_notes_tool_context_message(messages: List[Dict]) -> Optional[Dict]:
    ...
    return {
        "role": "user",
        "content": (
            "Recent SnapForge tool context for follow-up references only. "
...
def _minimal_snapforge_doc_messages(messages: List[Dict], active_document, stream_create: bool = False) -> List[Dict]:
    """Tiny prompt path for the SnapForge document LoRA.
    ...
    if stream_create:
        system = (
            "You are SnapForge. Create the requested document by streaming exactly one fenced block:\n"
            "```document\n"
            "Title\n"
            "markdown\n"
            "Document content\n"
        ...
        system = (
            "You are SnapForge. Edit or suggest changes to the active document using exactly one fenced tool block when needed.\n"
            ...
            "After the tool succeeds, SnapForge will answer Done."
        )
...
def _minimal_snapforge_notes_messages(messages: List[Dict]) -> List[Dict]:
    """Tiny prompt path for SnapForge notes/calendar/tasks LoRAs.

    The finetune is trained to emit SnapForge notes/calendar/task tool calls
    without receiving the full tool schema or saved-context wrapper stack.
    """
    latest = _extract_last_user_message(messages)
    system = (
        "You are SnapForge. Handle notes, reminders, calendar events, and scheduled tasks.\n"
...
def _minimal_snapforge_general_messages(messages: List[Dict], include_memory: bool = False) -> List[Dict]:
    """Minimal fallback for SnapForge finetunes outside domain-specific paths."""
    latest = _extract_last_user_message(messages)
    system = (
        "You are SnapForge. Answer directly and briefly.\n"
        "Use SnapForge tool-call format only when the user explicitly asks you to take an action.\n"
...
def _normalize_ody_qwen_text_artifacts(text: str) -> str:
    """Repair common dropped-final-letter artifacts from small SnapForge LoRAs.

    This is intentionally scoped to the snapforge-qwen3 runtime path. It is not
    ...
    if active_document:
        ...
        if _is_email_doc:
            ...
            doc_ctx = (
                ...
                f'2. Replace ONLY the new reply text above `---------- Previous message ----------`. You may omit the quoted history from your tool output; SnapForge preserves everything from that separator downward automatically.\n'
...
def _compute_final_metrics(
    ...
    # Context % should describe the prompt SnapForge assembled, not provider
...
async def stream_agent_loop(
    ...
    _ody_qwen_finetune_model = (model or "").lower().startswith("snapforge-qwen3")
    ...
    if _direct_low_signal:
        ...
        direct_messages = (
            _minimal_snapforge_general_messages(
                messages,
                include_memory=True,
            )
            if _ody_qwen_finetune_model
    ...
    if not guide_only and _relevant_tools is not None:
        ...
        if (
            ...
            logger.info("[tool-rag] Workspace file/terminal request; using SnapForge Terminus toolset")
    ...
    if _ody_doc_finetune_mode and _relevant_tools is not None:
        ...
        logger.info("[agent-intent] snapforge doc finetune tool clamp=%s", sorted(_relevant_tools))
        ...
        logger.info("[agent-intent] snapforge notes finetune tool clamp=%s", sorted(_relevant_tools))
        ...
        try:
            ...
            pass
        logger.info("[agent-intent] snapforge general no-tool clamp active")
    ...
    if _ody_doc_finetune_mode and not plan_mode and not approved_plan and not guide_only:
        messages = _minimal_snapforge_doc_messages(
            messages,
        ...
        mcp_schemas = []
        logger.info(
            "[agent-intent] snapforge doc minimal prompt active active_doc=%s stream_create=%s messages=%s",
        ...
        messages = _minimal_snapforge_notes_messages(messages)
        mcp_schemas = []
        logger.info(
            "[agent-intent] snapforge notes minimal prompt active messages=%s",
            len(messages),
        )
    elif _ody_qwen_finetune_model and not plan_mode and not approved_plan and not guide_only:
        messages = _minimal_snapforge_general_messages(
            messages,
            include_memory=True,
        )
        mcp_schemas = []
        logger.info(
            "[agent-intent] snapforge general minimal prompt active include_memory=%s messages=%s",
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        if _force_answer:
            ...
            # SnapForge-Qwen fine-tunes are trained to emit SnapForge tool calls
        ...
        if _ody_doc_stream_create_mode and tool_blocks:
            ...
            if create_idx is None:
                logger.info(
                    "[agent] snapforge doc stream-create discarded non-create tool call(s): %s",
                ...
                if len(tool_blocks) > 1 or create_idx != 0:
                    logger.info(
                        "[agent] snapforge doc stream-create keeping first create_document and dropping extras: %s",
                        [block.tool_type for block in tool_blocks],
                    )
        ...
        if _ody_qwen_finetune_model and tool_blocks:
            ...
            if _dropped_memory_lookup:
                logger.info(
                    "[agent-intent] snapforge qwen dropped manage_memory lookup; answering from compact memory"
                )
        ...
        if _doc_stream_create_completed:
            ...
            logger.info("[agent] snapforge doc stream-create completed after one create_document")
            break
        ...
        if _ody_doc_tool_completed:
            if not full_response.strip() or full_response.strip().startswith("```"):
                full_response = "Done."
                yield 'data: ' + json.dumps({"delta": "Done."}) + '\n\n'
            logger.info("[agent] snapforge doc tool completed after one textual tool block")
            break

        if (_ody_notes_finetune_mode or _ody_qwen_finetune_model) and _ody_notes_tool_completed:
            logger.info("[agent] snapforge completed from deterministic tool output")
            break

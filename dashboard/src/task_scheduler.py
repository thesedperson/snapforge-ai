...
class TaskScheduler:
        ...
        try:
            ...
            if gate_foreground:
                ...
                if waiting and waiting.status == "queued":
                    waiting.result = "Queued — waiting for SnapForge to be idle…"
                    db.commit()
                ...
                async def _cancel_if_foreground_active():
                    ...
                    while True:
                        ...
                        if has_foreground_activity():
                            ...
                            logger.info("Task '%s' interrupted because SnapForge became active", task.name)
                            if current_task:
                                current_task.cancel()
                            return
            ...
            try:
                ...
                msg = (
                    "Paused because SnapForge became active"
    ...
    async def _deliver_via_email(self, output: str, task, result: str):
        ...
        try:
            ...
            msg["X-SnapForge-Origin"] = "snapforge-ui"
            msg["X-SnapForge-Kind"] = "task"
            msg["X-SnapForge-Ref"] = str(task.id)
    ...
    async def _deliver_via_mcp(self, tool_name: str, task, result: str):
        ...
        args = {
            ...
            "body": result,
            "headers": {
                "X-SnapForge-Origin": "snapforge-ui",
                "X-SnapForge-Kind": "task",
                "X-SnapForge-Ref": str(task.id),
            },
        }
        if recipient:
    ...
    async def stop_background_tasks_for_foreground(self, *, reason: str = "SnapForge became active") -> int:
        ...
        user opens or uses SnapForge, foreground interaction wins immediately.

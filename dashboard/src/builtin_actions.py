...
async def action_run_script(owner: str, script: str = "", host: str = "", **kwargs) -> Tuple[str, bool]:
    ...
    target_host = (host or os.getenv("SNAPFORGE_SCRIPT_HOST", "localhost")).strip()
...
async def action_check_email_urgency(owner: str, **kwargs) -> Tuple[str, bool]:
    ...
    try:
                    ...
                    for uid_b in uids:
                        ...
                        try:
                            ...
                            if not raw:
                                continue
                            msg = _email_mod.message_from_bytes(raw)
                            # Skip SnapForge-generated reminders so the scanner
                            ...
                            _ody_origin = (msg.get("X-SnapForge-Origin") or "").strip().lower()
                            _ody_kind = (msg.get("X-SnapForge-Kind") or "").strip().lower()
                            ...
                            if (_ody_origin == "snapforge-ui" or _ody_kind == "reminder"
                                    or _raw_subj.startswith("reminder (snapforge):")
        ...
        if new_urgent:
            ...
            # straight into the SnapForge Email tab).
...
BUILTIN_ACTION_INFO = {
    ...
    "run_script": "Run a script locally or on SNAPFORGE_SCRIPT_HOST",

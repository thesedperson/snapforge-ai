...
# in SNAPFORGE_MCP_ALLOWED_COMMANDS -- a stdio server that genuinely needs an
...
def _mcp_allowed_commands() -> set:
    ...
    path. Empty by default; set SNAPFORGE_MCP_ALLOWED_COMMANDS (comma-separated)
    to opt specific trusted binaries in. Denied commands are rejected even if
    listed here."""
    raw = os.environ.get("SNAPFORGE_MCP_ALLOWED_COMMANDS", "")
...
def _validate_mcp_command(command, args, env) -> Optional[str]:
    ...
    if base not in _mcp_allowed_commands():
        return (
            ...
            "SNAPFORGE_MCP_ALLOWED_COMMANDS if you trust it, or register the "

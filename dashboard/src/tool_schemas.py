...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Manage calendar events: list events in a date range, create, update, delete. Each event can carry a tag/category (event_type) and importance level. Resolve relative dates like today/tomorrow against the 'Current date and time' system context, then pass ISO 8601 datetimes in the user's local wall time; for all-day events set all_day=true and pass YYYY-MM-DD. For event reminders/alarms, pass reminder_minutes; the tool creates the SnapForge note reminder, so do not also call manage_notes for the same reminder. Do not set rrule for single-occurrence requests such as 'next Wednesday only'; use rrule only when the user explicitly wants recurrence.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string",
                    ...
                    "reminder_minutes": {"type": "integer", "description": "For create_event: create an SnapForge reminder this many minutes before the event, e.g. 5 for 'reminder 5 min before'."},

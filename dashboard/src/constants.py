...
DATA_DIR = os.getenv("SNAPFORGE_DATA_DIR", get_default_data_dir())

# Data file paths
# Single source of truth: every persisted file/dir lives under DATA_DIR, which
# is the ONLY place SNAPFORGE_DATA_DIR is read. Import these constants instead of
...
MAIL_ATTACHMENTS_DIR = os.getenv("SNAPFORGE_MAIL_ATTACHMENTS_DIR", os.path.join(DATA_DIR, "mail-attachments"))
...
def internal_api_base() -> str:
    """Base URL for in-process loopback calls to SnapForge's own API.

    Agent tools and background jobs reach admin-gated routes by calling the
    running server over HTTP. Resolution order:
      1. SNAPFORGE_INTERNAL_BASE  - explicit override (e.g. behind a TLS proxy).
    ...
    """
    override = os.environ.get("SNAPFORGE_INTERNAL_BASE")
    if override:

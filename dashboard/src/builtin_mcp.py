...
MCP_DISABLED = os.environ.get("SNAPFORGE_DISABLE_MCP", "").lower() in ("1", "true", "yes")
BROWSER_MCP_REQUIRE_CACHE = os.environ.get("SNAPFORGE_BROWSER_MCP_REQUIRE_CACHE", "").lower() in ("1", "true", "yes")
...
def _find_browser_executable() -> str:
    ...
    """
    configured = os.environ.get("SNAPFORGE_BROWSER_EXECUTABLE", "").strip()
    if configured:
        return configured
...
def _browser_mcp_args(args: list[str]) -> list[str]:
    ...
    if os.environ.get("SNAPFORGE_BROWSER_ISOLATED", "1").lower() not in ("0", "false", "no"):
        if "--isolated" not in out and "--user-data-dir" not in out:
            out.append("--isolated")
    if os.environ.get("SNAPFORGE_BROWSER_NO_SANDBOX", "1").lower() not in ("0", "false", "no"):
...
async def register_builtin_servers(mcp_manager):
    ...
    if MCP_DISABLED:
        logger.info("Built-in MCP servers disabled via SNAPFORGE_DISABLE_MCP")
        return
    ...
    async def _start_npx_servers():
        ...
        for server_id, cfg in _BUILTIN_NPX_SERVERS.items():
            ...
            # with SNAPFORGE_BROWSER_MCP_REQUIRE_CACHE=1.
            ...
            if BROWSER_MCP_REQUIRE_CACHE and pkg_spec and not await _is_npx_package_cached(npx_path, pkg_spec):
                logger.warning(
                    ...
                    f"          (run once, then restart SnapForge)\n"
                    f"  Notes:  SNAPFORGE_BROWSER_MCP_REQUIRE_CACHE=1 is set, "
                    f"so SnapForge will not install browser automation on startup."
                )
                continue

...
async def _cookbook_hf_model_info(repo_id: str) -> Dict[str, Any]:
    ...
    not installed in SnapForge. Failures return a structured warning rather than
...
async def do_serve_model(content: str, owner: Optional[str] = None) -> Dict:
    ...
    try:
        ...
        if data.get("ok"):
            ...
            where = host or "local"
            log_path = f"/tmp/snapforge-tmux/{sid}.log"
            return {
                "output": (
    ...
    for t in merged:
        ...
        if t.get("status") == "error" and t.get("output_tail"):
            ...
            if tail:
                ...
                # banner ("Locale: C / Ubuntu_SnapForge ❯") — useless for
...
async def do_tail_serve_output(content: str, owner: Optional[str] = None) -> Dict:
    ...
    # Prefer the persisted /tmp/snapforge-tmux/SESSION.log file over the

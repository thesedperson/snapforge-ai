...
def _local_model_gate_enabled() -> bool:
    return os.getenv("SNAPFORGE_LOCAL_MODEL_GATE", "true").lower() not in {"0", "false", "no", "off"}
...
def _ollama_normalize_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt SnapForge' canonical OpenAI-style messages to native Ollama /api/chat.

    Two shape mismatches silently break requests:

    1. Tool calls: SnapForge carries `function.arguments` as a JSON *string*.
    ...
    2. Images (issue #4723): SnapForge carries multimodal user content as an
...
def _apply_local_cache_affinity(payload: Dict, url: str, session_id: Optional[str]) -> None:
    ...
    Sending a stable ``session_id`` (derived from the SnapForge session) lets
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/snapforge-dev/snapforge")
        h.setdefault("X-OpenRouter-Title", "SnapForge")
    if provider == "copilot":
...
# value returns HTTP 400. SnapForge does not currently send the `thinking` mode
...
# SNAPFORGE_MISTRAL_REASONING_EFFORT (e.g. set to "medium" for cheaper chat).
_MISTRAL_REASONING_EFFORT = os.getenv("SNAPFORGE_MISTRAL_REASONING_EFFORT", "high")
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip SnapForge-only metadata before sending messages to providers.
...
async def _stream_llm_inner(url: str, model: str, messages: List[Dict], temperature: float = LLMConfig.DEFAULT_TEMPERATURE,
    ...
    if provider == "anthropic":
        ...
        # Effort level is configurable via SNAPFORGE_MISTRAL_REASONING_EFFORT

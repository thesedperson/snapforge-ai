...
SnapForge stands on the shoulders of a lot of open-source work. This file
...
- **[llmfit](https://github.com/AlexsJones/llmfit)** by **Alex Jones** — the
  ...
  and `scripts/snapforge-cookbook`.
- **[Tongyi DeepResearch](https://github.com/Alibaba-NLP/DeepResearch)** by
  **Alibaba-NLP / Tongyi Lab** — the multi-step deep-research agent pipeline.
  Copyright © Alibaba-NLP / Tongyi Lab. **Apache-2.0.** Adapted for SnapForge's
...
and run alongside SnapForge on `docker compose up`. They are not modified —
just composed.
...
SnapForge talks to these over the network/API. They are **not** distributed
...
- **`caldav`** (Python lib) is **dual-licensed GPL-3.0-or-later OR Apache-2.0**.
  SnapForge uses it under **Apache-2.0**, which is permissive and MIT-compatible.
...
Most of SnapForge's code was written *with* AI models, not just by a human.

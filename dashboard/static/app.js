// ============================================
// SnapForge UI — Main Application Orchestrator
...
function _isForegroundChatBusy() {
  ...
  return !!window.__snapforgeChatBusy
    || Date.now() < (window.__snapforgeChatBusyUntil || 0)
...
function _submitMobileQueuedInput(input) {
  ...
  window.__snapforgeQueueStreamingSubmit = now;
...
try {
  const cachedDefaultChat = JSON.parse(localStorage.getItem('snapforge-default-chat-cache') || 'null');
  if (cachedDefaultChat && cachedDefaultChat.endpoint_url && cachedDefaultChat.model) {
    _defaultChat = cachedDefaultChat;
    window.__snapforgeDefaultChat = cachedDefaultChat;
  }
} catch (_) {}
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try {
        window.__snapforgeDefaultChat = d;
        localStorage.setItem('snapforge-default-chat-cache', JSON.stringify(d));
      } catch (_) {}
      return d;
    }
  } catch (_) {}
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'SnapForge Chat';
  ...
  if (_opener) window._snapforgeRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('snapforge-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  function applyModeToToggles(mode) {
    ...
    window.__snapforgeSetChatMode = setMode;
  ...
  (function initPlanToggle() {
    ...
    window.__snapforgeSetPlanMode = (active) => setPlanMode(active, { silent: true });
    const statusToggle = el('plan-mode-status-toggle');
    if (btn) {
      btn.addEventListener('click', () => {
        const st = loadToggleState();
    ...
    if (msgInput && !msgInput._snapforgePlanTabToggle) {
      msgInput._snapforgePlanTabToggle = true;
    ...
    if (chatBar && !chatBar._snapforgePlanSwipeToggle) {
      chatBar._snapforgePlanSwipeToggle = true;
      let touchStartX = 0;
      let touchStartY = 0;
      let touchStartAt = 0;
  ...
  const SPLASH_COUNT_KEY = 'snapforge-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    const TOOLBAR_HIDE_WIDTH = 160;
	    ...
	    function setComposerPlaceholder(width) {
	      ...
	      textarea.setAttribute('placeholder', width < PLACEHOLDER_COMPACT_WIDTH ? 'Message...' : 'Message SnapForge...');
	    }

	    if (_isMobile && textarea && !textarea._snapforgePlanPlaceholderHint) {
	      textarea._snapforgePlanPlaceholderHint = true;
	      setInterval(() => {
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (typeof window.__snapforgeSetChatMode === 'function') {
          window.__snapforgeSetChatMode('chat');
        } else {
        ...
        if (typeof window.__snapforgeSetChatMode === 'function') {
          window.__snapforgeSetChatMode(_restoreMode === 'chat' ? 'chat' : 'agent');
        }
  ...
  const UI_VIS_KEY = 'snapforge-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'snapforge-toolbar-visibility';
    try {
  ...
  if (textarea) {
    ...
    window.addEventListener('snapforge:chat-busy-change', () => _syncMobileEnterKeyHint(textarea));
      ...
      if (_shouldQueueFromMobileEnter(e, textarea) || (e.key === 'Enter' && !e.shiftKey && !e.isComposing && !isMobile)) {
        ...
        if (form) {
          if (_isForegroundChatBusy() && textarea.value && textarea.value.trim()) {
            ...
            window.__snapforgeQueueStreamingSubmit = Date.now();
          }
          _submitChatFormDirect(form);
        }
      }
...
// ============================================
function startSnapForgeApp() {
  tasksModule?.startNotificationPolling?.();
  if (window.__snapforgeAppStarted) return;
  window.__snapforgeAppStarted = true;
  const _bumpChatPriority = (ms = 10000) => {
    try {
      window.__snapforgeChatBusyUntil = Math.max(window.__snapforgeChatBusyUntil || 0, Date.now() + ms);
    } catch (_) {}
  };
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('snapforge-doc-open-' + _curSession) === '1') {
  ...
  if (messageInput && !messageInput._snapforgePromptRecallCapture) {
    messageInput._snapforgePromptRecallCapture = true;
    let recallHistory = [];
    let recallIndex = -1;
    let lastRecalled = '';
    ...
    messageInput.addEventListener('input', () => {
      ...
      recallIndex = -1;
      lastRecalled = '';
      try { delete messageInput.dataset.snapforgeRecallIndex; } catch {}
    }, true);
    messageInput.addEventListener('keydown', (e) => {
      ...
      if (current && currentIndex < 0) {
        const markedIndex = Number(messageInput.dataset.snapforgeRecallIndex);
      ...
      if (e.key === 'ArrowDown') {
        ...
        try { messageInput.dataset.snapforgeRecallIndex = String(nextIndex); } catch {}
  ...
  window._snapforgeBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sendBtn) {
    sendBtn.addEventListener('click', (e) => {
      ...
      if (sendBtn.dataset.mode === 'streaming') {
        if (hasText) window.__snapforgeQueueStreamingSubmit = Date.now();
        handleSubmit(e);
        return;
      }
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._snapforgeRouteOpener) {
          try { window._snapforgeRouteOpener(); } catch (_) {}
          window._snapforgeRouteOpener = null;
        }
      });
  } else {
  ...
  const runNonCriticalStartup = (fn, delay = 4000) => {
    let tries = 0;
    const run = () => {
      const busy = !!window.__snapforgeChatBusy
        || Date.now() < (window.__snapforgeChatBusyUntil || 0)
        || !!document.querySelector('.send-btn[data-mode="streaming"], .send-btn.send-pending');
      if (busy && tries < 12) {
        tries += 1;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startSnapForgeApp, { once: true });
} else {
  startSnapForgeApp();
}

...
const GROUP_STATE_KEY = 'snapforge-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const storedGroupSessions = Storage.getJSON('snapforge-group-sessions', []);
      const gids = Array.isArray(storedGroupSessions) ? storedGroupSessions : [];
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('snapforge-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {

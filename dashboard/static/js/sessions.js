...
let _sortMode = Storage.get('snapforge-session-sort') || 'active'; // default to last active
const DATE_SECTION_COLLAPSE_KEY = 'sf-session-date-section-collapsed';
let _autoCreateInProgress = false; // guard against recursive auto-create
const _INCOGNITO_SESSIONS_KEY = 'sf-incognito-sessions'; // sessionStorage key for incognito session IDs
...
function _shouldPreserveStartupComposer(msgInput) {
  ...
  if (window.__snapforgeComposerUserEdited) return true;
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'SnapForge Chat';
...
const FOLDER_STATE_KEY = 'snapforge-folder-state';
const FOLDER_ORDER_KEY = 'snapforge-folder-order';
...
export async function selectSession(id, { keepSidebar = false, showLoading = true, immediateLoading = false } = {}) {
  ...
  try {
    ...
    currentSessionId = id;
    try { window.__snapforgeLastSelectedSessionId = id; } catch (_) {}
    ...
    // URL hash — the user complained that coming back to SnapForge kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'SnapForge Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('snapforge-doc-open-' + id) === '1';
...
async function _getPreferredDefaultChat() {
  let dc = null;
  try {
    dc = window.__snapforgeDefaultChat || null;
  } catch (_) {}
  if (!dc || !dc.endpoint_url || !dc.model) {
    try {
...
export function createDirectChat(url, modelId, endpointId, opts = {}) {
  ...
  currentSessionId = null;
  try { window.__snapforgeLastSelectedSessionId = ''; } catch (_) {}
  Storage.remove('lastSessionId');
  history.replaceState(null, '', window.location.pathname);
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('snapforge-session-sort', mode);
  else Storage.remove('snapforge-session-sort');
  renderSessionList();
}

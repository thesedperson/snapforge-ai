    ...
    <div class="email-settings-section email-settings-style-section">
      <div class="email-settings-section-head">
        <div>
          ...
          <div class="email-settings-section-desc">Used when SnapForge drafts replies for you.</div>
        </div>
      </div>
      <label class="email-settings-field">
...
document.addEventListener('snapforge:email-filter-tag', (e) => {
...
try { window.__snapforgeGetActiveEmailContext = _getActiveEmailContext; } catch (_) {}
...
window.addEventListener('snapforge-reminder-channel-changed', (e) => {
...
const _LIB_SESSION_CACHE_PREFIX = 'snapforge.email.list.';
const _LIB_SESSION_CACHE_TTL_MS = 10 * 60 * 1000;
const _LIB_LAST_ACCOUNT_KEY = 'snapforge.email.lastAccountId';
...
function _isChatInteractionBusy() {
  try {
    if (window.__snapforgeChatBusy) return true;
    const until = Number(window.__snapforgeChatBusyUntil || 0);
...
function _publishActiveAccount() {
  try { window.__snapforgeActiveEmailAccount = state._libAccountId || null; } catch (_) {}
  try {
...
export function openEmailLibrary(opts = {}) {
        ...
        <div class="admin-card" style="flex:1;flex-direction:column;display:flex;overflow:hidden;">
          ...
          <div class="memory-toolbar">
            <div class="memory-category-filters">
              ...
              <button class="memory-toolbar-btn email-reminders-clear-btn hidden" id="email-reminders-clear-btn" title="Permanently delete SnapForge reminder emails">
            ...
            <div class="email-search-wrap" style="position:relative;flex:1;min-width:140px;">
              ...
              <button class="memory-toolbar-btn email-reminder-toggle-inline hidden" id="email-reminder-btn" title="Show SnapForge reminder emails">
  ...
  if (tagsToggle) {
    ...
    tagsToggle.addEventListener('click', () => {
      ...
      localStorage.setItem('snapforge.email.showTags', state._libShowTags ? '1' : '0');
      tagsToggle.classList.toggle('active', !!state._libShowTags);
      tagsToggle.setAttribute('aria-pressed', String(!!state._libShowTags));
      _renderGrid();
      document.dispatchEvent(new CustomEvent('snapforge:email-tags-toggle', { detail: { show: state._libShowTags } }));
    });
  }
  document.getElementById('email-reminders-clear-btn')?.addEventListener('click', async () => {
    const ok = await styledConfirm('Permanently delete all SnapForge reminder emails?', {
      confirmText: 'Delete',
      cancelText: 'Cancel',
      danger: true,
    });
    if (!ok) return;
    try {
      const res = await fetch(`${API_BASE}/api/email/snapforge/reminders?permanent=1${_acct()}`, {
        method: 'DELETE',
        credentials: 'same-origin',
      });
      const data = await res.json().catch(() => ({}));
...
const _BUBBLES_DISABLED_KEY = 'snapforge.email.bubblesDisabled';
...
const _SUMMARY_COLLAPSED_KEY = 'snapforge.email.summaryCollapsed';

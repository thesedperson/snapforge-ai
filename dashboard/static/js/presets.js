  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('snapforge-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('snapforge-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('snapforge-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('snapforge-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('snapforge-char-sessions', JSON.stringify(charSessions));

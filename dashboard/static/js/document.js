...
import { bindMenuDismiss, dismissOrRemove } from './escMenuStack.js';
  ...
  const _EMAIL_LOCAL_DRAFT_PREFIX = 'snapforge.email.replyDraft.v1:';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__snapforgeActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'snapforge-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'snapforge-doc-minimized-' + sessionId;
  ...
  let _snapforgeAttachMenu = null;

  function _closeSnapForgeAttachMenu() {
    if (_snapforgeAttachMenu) {
      _snapforgeAttachMenu.remove();
      _snapforgeAttachMenu = null;
    }
  ...
  function _attachMenuOutsideClick(e) {
    if (_snapforgeAttachMenu && !_snapforgeAttachMenu.contains(e.target)) _closeSnapForgeAttachMenu();
  }

  function _attachMenuEscape(e) {
    if (e.key !== 'Escape') return;
    _closeSnapForgeAttachMenu();
  }

  function _positionSnapForgeAttachMenu(anchor, menu) {
  ...
  function _snapforgeAttachLabel(item, kind) {
    if (kind === 'gallery') {
  ...
  async function _stageSnapForgeAttachment(kind, id) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-snapforge`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function _stageSnapForgeZip(items) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-snapforge-zip`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  function _afterSnapForgeAttachmentsAdded(count, label) {
    _renderComposeAttachments();
    clearTimeout(_autoSaveDebounce);
    _autoSaveDebounce = setTimeout(() => { saveDocument({ silent: true }); }, 800);
  ...
  async function _attachSnapForgeItem(kind, id, label, opts = {}) {
    try {
      const data = await _stageSnapForgeAttachment(kind, id);
      if (!data) return;
      _afterSnapForgeAttachmentsAdded(1, label || data.filename);
      if (!opts.keepOpen) _closeSnapForgeAttachMenu();
    } catch (err) {
      console.error('Failed to attach SnapForge item:', err);
      if (uiModule) uiModule.showError('Failed to attach from SnapForge');
    }
  }

  function _selectedSnapForgeAttachRows(menu) {
    return Array.from(menu?.querySelectorAll?.('.email-snapforge-attach-row.is-selected') || []);
  }

  function _syncSnapForgeAttachSelection(menu) {
    const selected = _selectedSnapForgeAttachRows(menu);
    const bar = menu?.querySelector?.('.email-snapforge-attach-actions');
    const count = menu?.querySelector?.('.email-snapforge-attach-count');
    const attachBtn = menu?.querySelector?.('.email-snapforge-attach-selected');
  ...
  async function _attachSelectedSnapForgeItems(menu) {
    const rows = _selectedSnapForgeAttachRows(menu);
    if (!rows.length) return;
    const btn = menu.querySelector('.email-snapforge-attach-selected');
    if (btn) {
      btn.disabled = true;
    ...
    try {
      ...
      if (zip) {
        await _stageSnapForgeZip(items);
        added = 1;
      } else {
        for (const item of items) {
          await _stageSnapForgeAttachment(item.kind, item.id);
          added += 1;
        }
      }
      _afterSnapForgeAttachmentsAdded(added, zip ? 'snapforge-attachments.zip' : undefined);
      _closeSnapForgeAttachMenu();
    } catch (err) {
      console.error('Failed to attach selected SnapForge items:', err);
      if (uiModule) uiModule.showError(added ? `Attached ${added}, then failed` : 'Failed to attach from SnapForge');
      _renderComposeAttachments();
    } finally {
      if (btn) {
  ...
  async function _loadSnapForgeAttachItems(menu, kind) {
    const list = menu.querySelector('.email-snapforge-attach-list');
    if (!list) return;
    ...
    const q = (menu.querySelector('.email-snapforge-attach-search')?.value || '').trim();
    try {
      ...
      if (!items.length) {
        list.innerHTML = `<div class="email-snapforge-attach-empty">${q ? 'No matches' : `No ${kind === 'gallery' ? 'images' : 'documents'}`}</div>`;
        _syncSnapForgeAttachSelection(menu);
        return;
      }
      list.innerHTML = '';
      for (const item of items) {
        const label = _snapforgeAttachLabel(item, kind);
        const row = document.createElement('button');
        row.type = 'button';
        row.className = `email-snapforge-attach-row ${kind === 'gallery' ? 'is-gallery' : ''}`;
        row.dataset.id = item.id || '';
        row.dataset.kind = kind;
        if (kind === 'gallery') {
          const src = item.url ? `${API_BASE}${item.url}` : '';
          row.innerHTML = `
            <span class="email-snapforge-attach-dot" aria-hidden="true"></span>
            <span class="email-snapforge-attach-thumb">${src ? `<img src="${_escHtml(src)}" alt="">` : ''}</span>
            <span class="email-snapforge-attach-main">
              <span class="email-snapforge-attach-title">${_escHtml(label)}</span>
              <span class="email-snapforge-attach-meta">${_escHtml(item.filename || 'image')}</span>
            </span>
          `;
        } else {
          row.innerHTML = `
            <span class="email-snapforge-attach-dot" aria-hidden="true"></span>
            <span class="email-snapforge-attach-icon">${langIcon(item.language || 'text', 14, { style: 'opacity:0.8;' })}</span>
            <span class="email-snapforge-attach-main">
              <span class="email-snapforge-attach-title">${_escHtml(label)}</span>
              <span class="email-snapforge-attach-meta">${_escHtml(item.language || 'text')}</span>
            </span>
          `;
        }
        ...
        row.addEventListener('dblclick', () => _attachSnapForgeItem(kind, item.id, label, { keepOpen: false }));
        list.appendChild(row);
      }
      _syncSnapForgeAttachSelection(menu);
    } catch (err) {
      console.error('Failed to load SnapForge attach items:', err);
      list.innerHTML = '<div class="email-snapforge-attach-empty">Could not load</div>';
    }
  }
  ...
  function _showComposeAttachMenu(anchor) {
    ...
    menu.className = 'email-snapforge-attach-menu';
    menu.innerHTML = `
      <button type="button" class="email-snapforge-attach-local">
      ...
      <div class="email-snapforge-attach-tabs">
      ...
      <label class="email-snapforge-attach-search-wrap">
        ...
        <input type="search" class="email-snapforge-attach-search" placeholder="Search attachments">
      </label>
      <div class="email-snapforge-attach-list"></div>
      <div class="email-snapforge-attach-actions">
        <span class="email-snapforge-attach-count"></span>
        <button type="button" class="email-snapforge-attach-selected" disabled>
    ...
    _snapforgeAttachMenu = menu;
    _positionSnapForgeAttachMenu(anchor, menu);
    menu.querySelector('.email-snapforge-attach-local')?.addEventListener('click', () => {
      _closeSnapForgeAttachMenu();
      document.getElementById('doc-email-file-input')?.click();
    });
    menu.querySelectorAll('[data-ody-attach-kind]').forEach(btn => {
      btn.addEventListener('click', () => _loadSnapForgeAttachItems(menu, btn.dataset.odyAttachKind));
    });
    let attachSearchTimer = null;
    menu.querySelector('.email-snapforge-attach-search')?.addEventListener('input', () => {
      clearTimeout(attachSearchTimer);
      attachSearchTimer = setTimeout(() => {
        _loadSnapForgeAttachItems(menu, menu.dataset.odyAttachKind || 'document');
      }, 220);
    });
    menu.querySelector('.email-snapforge-attach-selected')?.addEventListener('click', () => _attachSelectedSnapForgeItems(menu));
    setTimeout(() => {
      document.addEventListener('click', _attachMenuOutsideClick, true);
      document.addEventListener('keydown', _attachMenuEscape, true);
    }, 0);
    _loadSnapForgeAttachItems(menu, 'document');
  }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__snapforgeActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const _AI_REPLY_CONTEXT_STORE_PREFIX = 'snapforge:email-ai-reply-context:v1:';
  ...
  async function _aiReply(opts = {}) {
    ...
    const sourceUid = document.getElementById('doc-email-source-uid')?.value?.trim() || '';
    const sourceFolder = document.getElementById('doc-email-source-folder')?.value?.trim() || 'INBOX';
    const sourceAccountId = docs.get(activeDocId)?.sourceEmailAccountId || window.__snapforgeActiveEmailAccount || '';
  ...
  export function openPanel() {
    ...
    pane.innerHTML = `
      ...
      <div id="doc-email-actions" class="doc-email-actions" style="display:none">
        ...
        <div class="email-send-split">
          <button type="button" id="doc-email-send-btn" class="email-send-btn email-send-main" title="Send email (Ctrl+Enter)" onpointerdown="window.snapforgeEmailSendIntent&&window.snapforgeEmailSendIntent(event)" onmousedown="window.snapforgeEmailSendIntent&&window.snapforgeEmailSendIntent(event)" onclick="window.snapforgeEmailSendIntent&&window.snapforgeEmailSendIntent(event)"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>Send</button>
          <button type="button" id="doc-email-send-caret" class="email-send-btn email-send-caret" title="More send options" aria-haspopup="true" aria-expanded="false" onpointerdown="window.snapforgeEmailCaretIntent&&window.snapforgeEmailCaretIntent(event)" onmousedown="window.snapforgeEmailCaretIntent&&window.snapforgeEmailCaretIntent(event)"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9"/></svg></button>
    ...
    const handleSendIntent = (e) => {
      if (e && e.__snapforgeEmailSendHandled) return;
      ...
      if (e) {
        e.preventDefault();
        e.stopPropagation();
        e.__snapforgeEmailSendHandled = true;
      }
    ...
    window.snapforgeEmailSendIntent = handleSendIntent;
    ...
    const handleCaretIntent = (e) => {
      if (e && e.__snapforgeEmailCaretHandled) return;
      const now = Date.now();
      if (e && e.type === 'click' && now - lastCaretToggleAt < 350) {
        e.preventDefault();
        e.stopPropagation();
        e.__snapforgeEmailCaretHandled = true;
        return;
      }
      const rawTarget = e && e.target;
      const target = rawTarget && rawTarget.nodeType === Node.TEXT_NODE ? rawTarget.parentElement : rawTarget;
    ...
    window.snapforgeEmailCaretIntent = handleCaretIntent;
    ...
    let _fontIdx = parseInt(localStorage.getItem('snapforge-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('snapforge-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'snapforge-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('snapforge-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('snapforge-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('snapforge-suggestions-' + docId);
      if (!raw) return;

...
function _buildCrashReport(task, outputText) {
  ...
  const report = [
    '## SnapForge Cookbook crash report',
    '',
...
function _winSessionCmd(task, tmuxArgs) {
  const host = _taskRemoteHost(task);
  const sd = host ? '$env:TEMP\\snapforge-sessions' : '$env:TEMP\\snapforge-tmux';
  ...
  if (tmuxArgs.includes('capture-pane')) {
    ...
    const ps = host
      ...
      : `Get-Content (Join-Path $env:TEMP 'snapforge-tmux\\${sid}.log') -Tail ${lines} -ErrorAction SilentlyContinue`;
  ...
  if (tmuxArgs.includes('has-session')) {
    const ps = host
      ...
      : `$p = Get-Content (Join-Path $env:TEMP 'snapforge-tmux\\${sid}.pid') -ErrorAction SilentlyContinue; if ($p) { Get-Process -Id $p -ErrorAction SilentlyContinue | Out-Null; if ($?) { exit 0 } else { exit 1 } } else { exit 1 }`;
    return _winPowerShellCmd(task, ps);
  }
  ...
  if (tmuxArgs.includes('send-keys') && tmuxArgs.includes('C-c')) {
    const ps = host
      ...
      : `$p = Get-Content (Join-Path $env:TEMP 'snapforge-tmux\\${sid}.pid') -ErrorAction SilentlyContinue; if ($p) { Stop-Process -Id $p -ErrorAction SilentlyContinue }`;
    return _winPowerShellCmd(task, ps);
  }
...
function _winSessionStopTreePs(task) {
  ...
  return host
    ...
    : `${stopTree}; $p = Get-Content (Join-Path $env:TEMP 'snapforge-tmux\\${sid}.pid') -ErrorAction SilentlyContinue; if ($p -match '^\\d+$') { Stop-Tree ([int]$p) }; Remove-Item (Join-Path $env:TEMP 'snapforge-tmux\\${sid}.*') -Force -ErrorAction SilentlyContinue`;
}
  ...
  if (task.payload) {
    if (task.type === 'serve' && task.payload._cmd) {
      ...
      _updateTask(task.sessionId, {
        status: 'running',
        output: `${task.output || ''}\n\n[snapforge] Retrying download. Progress may briefly look like a fresh download while HuggingFace checks cached/incomplete files; cached partial files will be reused when available.`.trim(),
        _retrying: true,
      });
      ...
      const _openTaskMenu = (e) => {
        ...
        if (_isWindows(task)) {
          ...
          const logCmd = host
            ...
            : `powershell -Command "Get-Content (Join-Path $env:TEMP 'snapforge-tmux\\${task.sessionId}.log') -Wait"`;
...
const BG_LEADER_KEY = 'snapforge-cookbook-bg-leader';
...
function _foregroundChatBusy() {
  try {
    return !!window.__snapforgeChatBusy || Date.now() < (window.__snapforgeChatBusyUntil || 0);
  } catch {
    return false;
  }
}

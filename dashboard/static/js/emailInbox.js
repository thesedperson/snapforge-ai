...
function _openEmailTagFilter(tag) {
  ...
  setTimeout(() => {
    document.dispatchEvent(new CustomEvent('snapforge:email-filter-tag', { detail: { tag: normalized } }));
  }, 0);
}
...
let _showEmailTags = localStorage.getItem('snapforge.email.showTags') !== '0';

export function init(documentModule) {
  _docModule = documentModule;
  _bindEvents();
  document.addEventListener('snapforge:email-tags-toggle', (e) => {
...
function _bindEvents() {
  ...
  // Delay the lightweight unread badge check so opening SnapForge doesn't
...
async function _refreshUnreadCount() {
  ...
  try {
    ...
    const lastSeen = parseInt(localStorage.getItem('snapforge-email-last-seen-uid') || '0', 10);
  ...
  try {
    ...
    fetch(emailApiUrl('/api/email/unread-state', { folder: 'INBOX' }))
      ...
      .then(data => {
        const maxUid = parseInt(data.max_uid || '0', 10) || 0;
        if (maxUid > 0) {
          localStorage.setItem('snapforge-email-last-seen-uid', String(maxUid));
        }
        const dot = document.getElementById('email-unread-dot');
        if (dot) dot.style.display = 'none';
      })
...
async function _openEmail(em, itemEl, preloadedData = null, mode = 'reply', noteHint = '', prefilledBody = '') {
  ...
  try {
    ...
    if (wantsAiReply) {
      const activeReplyAccount = data.account_id || em.account_id || window.__snapforgeActiveEmailAccount || '';

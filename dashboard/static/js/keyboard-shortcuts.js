...
export function initKeyboardShortcuts(modules) {
  ...
  window._snapforgeKeybinds = { ..._defaultKeybinds };

  // Load saved keybinds
  fetch('/api/auth/settings', { credentials: 'same-origin' })
    .then(r => r.json())
    .then(s => { if (s.keybinds) window._snapforgeKeybinds = { ..._defaultKeybinds, ...s.keybinds }; })
    .catch(() => {});
  ...
  document.addEventListener('keydown', (e) => {
    const kb = window._snapforgeKeybinds;
      ...
      uiModule.styledConfirm('Delete this session?', { confirmText: 'Delete', danger: true }).then(ok => {
        ...
        fetch(`${API_BASE}/api/session/${sid}`, { method: 'DELETE' }).then(async () => {
          ...
          if (nextSession) {
            ...
            el('current-meta').textContent = 'SnapForge AI Chat';

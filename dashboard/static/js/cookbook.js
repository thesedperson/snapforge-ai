  ...
  try {
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (pkg.installed && pkg.pip_update_available === false && pkg.name !== 'llama_cpp') {
        const tip = esc(pkg.update_note || pkg.status_note || 'Found externally; update outside SnapForge.');
    ...
    const _appDepsHtml = (items) => {
      ...
      const parts = [_sectionHeader('SnapForge app', 'Run inside the SnapForge app itself.')];
      const order = ['System', 'Tools', 'Image', 'LLM', 'Audio', 'Other'];
      for (const cat of order) {
        const catItems = _sortDeps(byCat.get(cat) || [], cat);
        if (!catItems.length) continue;
  ...
  if (dlBtn && dlInput) {
    ...
    const triggerDownload = async () => {
      ...
      if (looksGgufRepo && !pickerInclude) {
        ...
        if (!pickerInclude) {
          uiModule.showToast('Pick a GGUF quant first. SnapForge will not download the whole GGUF repo without an include pattern.');
          return;
        }
...
function _renderRecipes() {
  ...
  html += '<div class="hwfit-list-footer" style="display:none;">'
       + 'Don\'t see a model? '
       + '<a href="https://github.com/snapforge-dev/snapforge/discussions/1962" target="_blank" rel="noopener" style="color:var(--accent,var(--red));text-decoration:none;display:inline-flex;align-items:center;gap:4px;vertical-align:middle;position:relative;top:-1px;">'
       + 'Request it →'
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Optional packages that extend SnapForge capabilities.</p>';
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install SnapForge keys, choose model directories, and set the default server. Local is this machine.</p>';
...
const SHARED_STATE_LEADER_KEY = 'snapforge-cookbook-shared-state-leader';

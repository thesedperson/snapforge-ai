...
const _ACTION_RECENTS_KEY = 'snapforge-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'snapforge-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'SnapForge';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'SnapForge' : modelRouteLabel(replyModels.requestedModel, resolvedModel);

...
export function wireArrowUpRecall(composer, getUserMessages, options = {}) {
  ...
  const debug = (...args) => {
    try {
      if (localStorage.getItem('snapforgeArrowRecallDebug') === '1') {
  ...
  composer.addEventListener('input', () => {
    ...
    lastRecalledValue = '';
    recallHistory = [];
    try { delete composer.dataset.snapforgeRecallIndex; } catch (_) {}
  });
  ...
  composer.addEventListener('keydown', (e) => {
    ...
    if (currentIndex < 0 && currentValue) {
      const markedIndex = Number(composer.dataset?.snapforgeRecallIndex);
    ...
    if (e.key === 'ArrowDown') {
      ...
      try { composer.dataset.snapforgeRecallIndex = String(nextIndex); } catch (_) {}

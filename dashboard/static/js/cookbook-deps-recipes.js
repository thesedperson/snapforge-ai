    ...
    variants: {
      pip: {
        commands: [
          ...
          'BRIDGE_DIR="${SNAPFORGE_ROOT:-$PWD}/swift/snapforge-mlx-image-bridge"; test -d "$BRIDGE_DIR" || { echo "Run this from an SnapForge checkout that includes swift/snapforge-mlx-image-bridge, or set SNAPFORGE_ROOT=/path/to/snapforge."; exit 1; }',
          'BRIDGE_DIR="${SNAPFORGE_ROOT:-$PWD}/swift/snapforge-mlx-image-bridge"; cd "$BRIDGE_DIR" && swift build -c release --product snapforge-mlx-inpaint',
          'BRIDGE_DIR="${SNAPFORGE_ROOT:-$PWD}/swift/snapforge-mlx-image-bridge"; mkdir -p "$HOME/.local/bin" && cp "$BRIDGE_DIR/.build/release/snapforge-mlx-inpaint" "$HOME/.local/bin/snapforge-mlx-inpaint"',
    ...
    variants: {
      pip: {
        commands: [
          'python -m pip install -U fastapi uvicorn python-multipart pillow huggingface_hub',
          'BRIDGE_DIR="${SNAPFORGE_ROOT:-$PWD}/swift/snapforge-mlx-image-bridge"; test -d "$BRIDGE_DIR" || { echo "Run this from an SnapForge checkout that includes swift/snapforge-mlx-image-bridge, or set SNAPFORGE_ROOT=/path/to/snapforge."; exit 1; }',
          'BRIDGE_DIR="${SNAPFORGE_ROOT:-$PWD}/swift/snapforge-mlx-image-bridge"; cd "$BRIDGE_DIR" && swift build -c release --product snapforge-mlx-colorize',
          'BRIDGE_DIR="${SNAPFORGE_ROOT:-$PWD}/swift/snapforge-mlx-image-bridge"; mkdir -p "$HOME/.local/bin" && cp "$BRIDGE_DIR/.build/release/snapforge-mlx-colorize" "$HOME/.local/bin/snapforge-mlx-colorize"',
          'MLX_METALLIB="$(python - <<\'PY\'\nimport pathlib, sys\ntry:\n    import mlx\nexcept Exception as exc:\n    raise SystemExit(f"mlx Python package is required for mlx.metallib: {exc}")\nroot = pathlib.Path(mlx.__file__).resolve().parent\nfor name in ("lib/mlx.metallib", "mlx.metallib", "lib/default.metallib", "default.metallib"):\n    path = root / name\n    if path.exists():\n        print(path)\n        break\nelse:\n    raise SystemExit(f"No MLX metallib found under {root}")\nPY\n)"; mkdir -p "$HOME/.local/bin" && cp "$MLX_METALLIB" "$HOME/.local/bin/mlx.metallib" && cp "$MLX_METALLIB" "$HOME/.local/bin/default.metallib"',
        ],
      },
    },

// SnapForge UI — Initialization Scripts
...
function markComposerUserEdited() {
  ...
  msgInput.addEventListener('input', () => {
    window.__snapforgeComposerUserEdited = !!msgInput.value;
  });
}
...
function clearFreshComposerRestore() {
  ...
  if (window.__snapforgeComposerUserEdited || document.activeElement === msgInput) return;
  if (msgInput.value) {
    msgInput.value = '';
...
(async () => {
  try {
    ...
    if (!liveUser) return;
    const KEY = 'snapforge-auth-user';
    const cachedUser = localStorage.getItem(KEY);
    if (cachedUser && cachedUser !== liveUser) {
      const _keepKeys = new Set(['snapforge-last-user', KEY]);
      const toRemove = [];

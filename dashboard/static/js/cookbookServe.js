...
function _formatServeCmdPreview(cmd) {
  ...
  if (mlxDeepSeekV4Compat) {
    ...
    if (homeMatch && shortName) {
      const shimPath = `${homeMatch[1]}/.cache/snapforge/mlx-shims/${shortName}`;
      raw = raw.replace(
  ...
  if (mlxDeepSeekV4Compat) {
    return [
      '# SnapForge runtime compatibility: using sanitized MLX DeepSeek-V4 shim.',
      formatted,
    ].join('\n');
  }
  return formatted;
...
function _rerenderCachedModels() {
  ...
  list.querySelectorAll('.memory-item[data-repo]').forEach(item => {
    item.addEventListener('click', async (e) => {
      ...
      panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="prefix_cache"${sv('prefix_cache',false)?' checked':''} /> Prefix / Radix Cache${_h('vLLM: prefix caching. SGLang: RadixAttention prefix cache; when off SnapForge adds --disable-radix-cache.')}</label>`;
      ...
      // reflects the stored presets. Standard SnapForge .dropdown look, positioned

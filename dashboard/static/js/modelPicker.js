...
const RECENT_KEY = 'snapforge-model-recent';
const FAVORITES_KEY = 'snapforge-model-favorites';
const RECENT_MAX = 5;
...
function _initModelPickerDropdown() {
  ...
  async function _refreshLocalProbe() {
    try {
      if (window.__snapforgeChatBusy || Date.now() < (window.__snapforgeChatBusyUntil || 0)) return;
    } catch (_) {}
    const now = Date.now();
  ...
  const _collapsedProviders = new Set(_loadList('snapforge-model-collapsed'));
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('snapforge-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
...
async function _pick(m) {
    _defaultPendingSeq++;
    try {
      window.__snapforgeLastPickedRoute = {
        model: m.mid || '',
        endpoint_url: m.url || '',
    ...
    try { window.__snapforgeModelSwitchPromise = switchPromise; } catch (_) {}
    const finishSwitch = () => {
      try {
        if (switchDone) switchDone();
        if (window.__snapforgeModelSwitchPromise === switchPromise) delete window.__snapforgeModelSwitchPromise;
      } catch (_) {}
    };
    ...
    try { document.dispatchEvent(new CustomEvent('snapforge:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('snapforge:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  if (!modelId && !currentSessionId && !_pendingChat && _deps.setPendingChat) {
    ...
    try {
      cachedDefault = window.__snapforgeDefaultChat || null;
    } catch (_) {}
    if (!cachedDefault || !cachedDefault.endpoint_url || !cachedDefault.model) {
      try {
        cachedDefault = JSON.parse(localStorage.getItem('snapforge-default-chat-cache') || 'null');
      } catch (_) {}
    }
    if (cachedDefault && cachedDefault.endpoint_url && cachedDefault.model) {
  ...
  // SECURITY: deliberately NOT auto-injecting `snapforge-model-favorites[0]`

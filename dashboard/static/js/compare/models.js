...
function _persistSelections() {
  if (state._selectedModels.length > 0) {
    Storage.setJSON('snapforge-compare-selections-' + (state._compareMode || 'chat'), state._selectedModels);
  }
  if ((state._compareMode === 'search' || state._compareMode === 'research') && state._searchSynthModels) {
    Storage.setJSON('snapforge-compare-synth-' + state._compareMode, state._searchSynthModels);
  }
}
...
const POOL_STORAGE_KEY = 'snapforge-shuffle-pool-excluded';

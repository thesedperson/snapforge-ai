...
async function showModelSelector() {
  return new Promise((resolve) => {
    ...
    const _selKey = 'snapforge-compare-selections-' + (state._compareMode || 'chat');
    let selections = Storage.getJSON(_selKey) || Storage.getJSON('snapforge-compare-selections') || [];
    // Restore synthesis models for search/research
    if (state._compareMode === 'search' || state._compareMode === 'research') {
      const savedSynth = Storage.getJSON('snapforge-compare-synth-' + state._compareMode);

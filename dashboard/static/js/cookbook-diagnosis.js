...
function _gpuCleanupCommand() {
  return `set -u
echo "[snapforge] Clearing GPU compute processes..."
if command -v nvidia-smi >/dev/null 2>&1; then
  pids="$(nvidia-smi --query-compute-apps=pid --format=csv,noheader,nounits 2>/dev/null | tr -d " " | grep -E "^[0-9]+$" | sort -u)"
  if [ -z "$pids" ]; then
    echo "[snapforge] No NVIDIA compute processes found."
    exit 0
  fi
  echo "[snapforge] GPU PIDs: $pids"
  ps -fp $pids 2>/dev/null || true
  echo "[snapforge] Sending TERM..."
  kill -TERM $pids || true
  sleep 3
  alive=""
  for pid in $pids; do
  ...
  if [ -n "$alive" ]; then
    echo "[snapforge] Force killing remaining GPU PIDs:$alive"
  ...
  if [ -n "$remaining" ]; then
    echo "[snapforge] GPU processes still remain:"
    echo "$remaining"
    exit 2
  fi
  echo "[snapforge] GPU cleanup complete. No NVIDIA compute processes remain."
else
  echo "[snapforge] nvidia-smi not found; falling back to common model-server process cleanup."
  ...
  echo "[snapforge] Fallback cleanup complete."
fi`;
}
...
export const ERROR_PATTERNS = [
  ...
  },
  {
    ...
    fixes: [
      { label: 'Open Dependencies', action: () => _openCookbookDependencies('vllm') },
      {
        ...
        action: () => _copyText('Upgrade the vLLM environment that provides the selected vllm CLI, or use a compatible checkpoint. Do not assume SnapForge owns PATH/system/source/Docker installs.'),
      },
    ],
  },
  {
  ...
  },
  {
    ...
    suggestion: 'Suggested action: relaunch with SnapForge’ venv CUDA library path fix. If the venv does not contain the matching NVIDIA runtime libs, run Repair sglang-kernel.',
    fixes: [
  ...
  },
  {
    ...
    suggestion: 'Suggested action: relaunch from the cached local snapshot path. SnapForge now rewrites MLX repo-id launches to the newest local Hugging Face snapshot when it exists on the selected Mac.',
    fixes: [
      { label: 'Edit / relaunch serve', action: (panel) => _openServeEditFromDiagnosis(panel) },
      { label: 'Open Dependencies', action: () => _openCookbookDependencies('mlx_lm') },
      { label: 'Copy error', action: (panel) => {
...
function _diagnosisCopyBundle(task, diagnosis, sourceText, suggestionText) {
  const lines = ['## SnapForge Cookbook troubleshooting'];
  if (task) {
    lines.push(
      '',
      '### Task',

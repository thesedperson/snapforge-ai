...
async function _selectAddedModelInChat(endpoint) {
  ...
  try {
    document.dispatchEvent(new CustomEvent('snapforge:auto-select-model', {
      detail: {
...
function initEndpointForm() {
  ...
  document.querySelectorAll('.adm-quickstart-section').forEach((sec) => {
    ...
    if (!head) return;
    const key = 'snapforge.addModels.' + sec.id + '.open';
    let open = false;
  ...
  try {
    ...
    list.querySelectorAll('[data-adm-del-token]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try { window.dispatchEvent(new CustomEvent('snapforge-integrations-changed')); } catch (_) {}
      });
    });
...
function initBackup() {
  el('adm-exportDataBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const filename = match ? match[1] : 'snapforge_backup.json';

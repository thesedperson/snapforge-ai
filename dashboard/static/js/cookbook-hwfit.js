...
function _renderHwVisibilityWarning(sys) {
  ...
  box.querySelector('[data-hw-action="copy"]')?.addEventListener('click', () => {
    ...
    const text = [
      'SnapForge Cookbook hardware diagnostics',
      ...
      'docker compose exec snapforge cat /proc/meminfo | head',
      'docker compose exec snapforge python -c "from services.hwfit.hardware import detect_system; import json; print(json.dumps(detect_system(fresh=True), indent=2))"',
    ].join('\n');
...
export function _hwfitInit() {
  ...
  function _syncServers() {
    ...
    // dropdown still showed snapforge. The user's selection must only change via

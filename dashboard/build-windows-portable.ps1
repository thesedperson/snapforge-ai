...
<#
  Build a portable Windows distribution for SnapForge.

  Output layout:
    dist\SnapForge\SnapForge.exe
    dist\SnapForge\static\...
    dist\SnapForge\scripts\...
    dist\SnapForge\mcp_servers\...
    dist\SnapForge\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name SnapForge @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\SnapForge" -ForegroundColor Green

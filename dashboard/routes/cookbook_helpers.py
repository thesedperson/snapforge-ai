...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When SnapForge runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends SnapForge' own interpreter directory to PATH. If SnapForge itself is
...
def _append_pip_install_runner_lines(runner_lines: list[str], cmd: str) -> None:
    ...
    runner_lines.append('  echo "[snapforge] pip does not support --break-system-packages; installing without it."')
...
def _user_shell_path_bootstrap() -> list[str]:
    return [
        'SNAPFORGE_USER_SHELL="${SHELL:-}"',
        'if [ -n "$SNAPFORGE_USER_SHELL" ] && [ -x "$SNAPFORGE_USER_SHELL" ]; then',
        '  SNAPFORGE_USER_PATH="$("$SNAPFORGE_USER_SHELL" -ic \'printf "__SNAPFORGE_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__SNAPFORGE_PATH__//p\' | tail -n 1 || true)"',
        '  if [ -n "$SNAPFORGE_USER_PATH" ]; then export PATH="$SNAPFORGE_USER_PATH:$PATH"; fi',
        'fi',
...
def _cached_model_scan_script(model_dirs: list[str] | None = None, add_hf_cache: str | None = None) -> str:
    ...
    lines = [
        ...
        "    if os.name == 'nt' and not os.environ.get('SNAPFORGE_ALLOW_OLLAMA_CLI_SCAN'): return",
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by SnapForge.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$SNAPFORGE_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $SNAPFORGE_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        ...
        runner_lines.append('  exit "$SNAPFORGE_PREFLIGHT_EXIT"')
    runner_lines.append('fi')
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for SnapForge-managed `pip install --user`
    # installs, but then report the actual CLI path so external runtimes are clear.
    runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
    runner_lines.append('SNAPFORGE_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"')
    runner_lines.append('if [ -z "$SNAPFORGE_VLLM_BIN" ]; then')
    runner_lines.append('  echo "ERROR: vLLM is not installed."')
    runner_lines.append('  SNAPFORGE_PREFLIGHT_EXIT=127')
    runner_lines.append('else')
    runner_lines.append('  echo "[snapforge] vLLM CLI: $SNAPFORGE_VLLM_BIN"')
    runner_lines.append('  SNAPFORGE_VLLM_VERSION="$("$SNAPFORGE_VLLM_BIN" --version 2>&1 | head -n 1 || true)"')
    runner_lines.append('  if [ -n "$SNAPFORGE_VLLM_VERSION" ]; then echo "[snapforge] vLLM version: $SNAPFORGE_VLLM_VERSION"; fi')
    runner_lines.append('fi')
...
def _append_serve_exit_code_lines(
    ...
    *,
    ...
    runner_lines.append('SNAPFORGE_CMD_EXIT=$?')
    if is_pip_install:
        runner_lines.append('if [ $SNAPFORGE_CMD_EXIT -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; fi')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $SNAPFORGE_CMD_EXIT ==="')
        ...
        runner_lines.append('echo ""; echo "=== Process exited with code $SNAPFORGE_CMD_EXIT ==="')
        runner_lines.append('exit "$SNAPFORGE_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    # every OS-package dep from the launch path. Sets _snapforge_have_prebuilt=1
    ...
    runner_lines.append('    _snapforge_have_prebuilt=""')
    runner_lines.append('    _snapforge_arch="$(uname -m)"')
    runner_lines.append('    _snapforge_prebuilt_url=""')
    runner_lines.append('    if command -v curl >/dev/null 2>&1 && [ "$_snapforge_arch" = "x86_64" ]; then')
    runner_lines.append('      _snapforge_pat=""')
    runner_lines.append('      _snapforge_has_nv_inline() { command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -L 2>/dev/null | grep -q "GPU "; }')
    runner_lines.append('      _snapforge_has_vk_inline() { ldconfig -p 2>/dev/null | grep -q "libvulkan\\.so" || command -v vulkaninfo >/dev/null 2>&1 || [ -e /usr/lib/x86_64-linux-gnu/libvulkan.so.1 ]; }')
    runner_lines.append('      _snapforge_has_vkdev_inline() { ls /dev/dri/renderD* >/dev/null 2>&1 || (lspci 2>/dev/null | grep -Ei \'VGA|3D|Display\' | grep -Eiq \'AMD|ATI|Radeon\'); }')
    runner_lines.append('      if _snapforge_has_nv_inline; then')
    runner_lines.append('        _snapforge_pat="ubuntu.*cuda"')
    runner_lines.append('      elif _snapforge_has_vkdev_inline && _snapforge_has_vk_inline; then')
    runner_lines.append('        _snapforge_pat="ubuntu.*vulkan"')
    runner_lines.append('      else')
    runner_lines.append('        _snapforge_pat="ubuntu-x64\\\\.zip"')
    runner_lines.append('      fi')
    runner_lines.append('      _snapforge_prebuilt_url="$(curl -fsSL --max-time 15 https://api.github.com/repos/ggml-org/llama.cpp/releases/latest 2>/dev/null | grep \'"browser_download_url"\' | cut -d\'"\' -f4 | grep -iE "$_snapforge_pat" | grep -iv "arm\\|aarch64" | head -1)"')
    runner_lines.append('    fi')
    ...
    runner_lines.append('    if [ -n "$_snapforge_prebuilt_url" ] && (command -v unzip >/dev/null 2>&1 || command -v bsdtar >/dev/null 2>&1 || command -v python3 >/dev/null 2>&1); then')
    runner_lines.append('      echo "[snapforge] Found prebuilt llama-server: $_snapforge_prebuilt_url"')
    runner_lines.append('      mkdir -p ~/bin "$HOME/.cache/snapforge/llama-cpp-prebuilt" && cd "$HOME/.cache/snapforge/llama-cpp-prebuilt"')
    runner_lines.append('      rm -f llama-cpp.zip')
    runner_lines.append('      if curl -fsSL --max-time 120 "$_snapforge_prebuilt_url" -o llama-cpp.zip && [ -s llama-cpp.zip ]; then')
    ...
    runner_lines.append('        _snapforge_extracted="$(find build -type f -name llama-server 2>/dev/null | head -1)"')
    runner_lines.append('        if [ -n "$_snapforge_extracted" ]; then')
    runner_lines.append('          chmod +x "$_snapforge_extracted"')
    runner_lines.append('          ln -sf "$_snapforge_extracted" ~/bin/llama-server')
    runner_lines.append('          _snapforge_libdir="$(dirname "$_snapforge_extracted")"')
    runner_lines.append('          mkdir -p ~/.config && echo "export LD_LIBRARY_PATH=\\"$_snapforge_libdir:\\${LD_LIBRARY_PATH:-}\\"" > ~/.config/snapforge-llama-cpp-env')
    runner_lines.append('          _snapforge_have_prebuilt=1')
    runner_lines.append('          echo "[snapforge] Prebuilt llama-server installed at $_snapforge_extracted"')
    runner_lines.append('        fi')
    runner_lines.append('      fi')
    runner_lines.append('      [ -z "$_snapforge_have_prebuilt" ] && echo "[snapforge] Prebuilt download/extract failed — falling back to from-source build."')
    runner_lines.append('    elif [ -z "$_snapforge_prebuilt_url" ]; then')
    runner_lines.append('      echo "[snapforge] No matching prebuilt llama-server for this host (arch=$_snapforge_arch) — will build from source."')
    runner_lines.append('    fi')
    runner_lines.append('  if [ -z "$_snapforge_have_prebuilt" ]; then')
    ...
    runner_lines.append('    _snapforge_has_nvidia_hw() {')
    ...
    runner_lines.append('    }')
    runner_lines.append('    if _snapforge_has_nvidia_hw; then')
    ...
    runner_lines.append('    _snapforge_apt_bootstrap() {')
    ...
    runner_lines.append('        echo "[snapforge] Auto-installing missing build deps via apt:$_missing"')
    ...
    runner_lines.append('        echo "[snapforge] Auto-installing missing build deps via pacman:$_missing"')
    ...
    runner_lines.append('        echo "[snapforge] Auto-installing missing build deps via dnf:$_missing"')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[snapforge] WARNING: missing build deps ($_missing) — passwordless sudo is unavailable, cannot auto-install. Cookbook Diagnosis will explain the fix after the build fails."')
    runner_lines.append('      fi')
    runner_lines.append('    }')
    runner_lines.append('    _snapforge_apt_bootstrap')
    runner_lines.append('    _snapforge_missing_build_deps=""')
    runner_lines.append('    command -v cmake >/dev/null 2>&1 || _snapforge_missing_build_deps="$_snapforge_missing_build_deps cmake"')
    runner_lines.append('    command -v git >/dev/null 2>&1 || _snapforge_missing_build_deps="$_snapforge_missing_build_deps git"')
    runner_lines.append('    command -v g++ >/dev/null 2>&1 || command -v gcc >/dev/null 2>&1 || _snapforge_missing_build_deps="$_snapforge_missing_build_deps build-essential"')
    runner_lines.append('    if [ -n "$_snapforge_missing_build_deps" ]; then')
    runner_lines.append('      echo "ERROR: llama.cpp source build needs missing packages:$_snapforge_missing_build_deps"')
    ...
    runner_lines.append('    _snapforge_has_vulkan() {')
    ...
    runner_lines.append('      return 1')
    runner_lines.append('    }')
    runner_lines.append('    _snapforge_has_vulkan_device() {')
    ...
    runner_lines.append('      fi')
    runner_lines.append('      echo "[snapforge] ROCm/HIP detected — building llama-server with HIP support..."')
    ...
    runner_lines.append('    elif command -v nvcc &>/dev/null && _snapforge_has_nvidia_hw; then')
    runner_lines.append('      rm -rf build')
    ...
    runner_lines.append('      _snapforge_has_cudart() {')
    ...
    runner_lines.append('        return 1')
    runner_lines.append('      }')
    runner_lines.append('      if _snapforge_has_cudart; then')
    runner_lines.append('        echo "[snapforge] CUDA nvcc + cudart found — building llama-server with CUDA (GPU) support..."')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[snapforge] WARNING: nvcc found but CUDA runtime (libcudart.so) is not visible — building llama-server for CPU only."')
    runner_lines.append('        echo "[snapforge]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('        echo "[snapforge]   Ensure libcudart is installed (e.g. cuda-runtime package) and visible via ldconfig or CUDA_HOME."')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    elif _snapforge_has_vulkan_device && _snapforge_has_vulkan; then')
    runner_lines.append('      echo "[snapforge] Vulkan-capable GPU detected (no ROCm/CUDA toolchain installed) — building llama-server with Vulkan support..."')
    ...
    runner_lines.append('    else')
    runner_lines.append('      echo "[snapforge] WARNING: no HIP/CUDA/Vulkan toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[snapforge]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[snapforge]   Install Vulkan (libvulkan-dev) / ROCm for AMD GPUs or CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      rm -rf build')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
    runner_lines.append('  fi  # end _snapforge_have_prebuilt guard')
...
def _llama_cpp_rebuild_cmd(update_source: bool = False) -> str:
    ...
    if update_source:
        update_cmd = (
            ...
            'echo "[snapforge] WARNING: llama.cpp source update failed; clearing cached build anyway."; '
            'elif command -v git >/dev/null 2>&1; then '
            'git clone --depth 1 https://github.com/ggml-org/llama.cpp "$HOME/llama.cpp" || '
            'echo "[snapforge] WARNING: llama.cpp clone failed; clearing cached build anyway."; '
            'fi && '
        )
    return (
        'mkdir -p "$HOME/bin" && '
        ...
        'echo "[snapforge] Cleared the cached llama.cpp build. '
...
WIN_SESSION_DIR = "$env:TEMP\\\\snapforge-sessions"
...
def _diagnose_serve_output(text: str) -> dict | None:
    ...
    patterns = [
        ...
        (
            r"mlx-lama-swift|snapforge-mlx-inpaint|mlx-lama-serve|LaMa / MI-GAN MLX inpainting models require",
            "LaMa / MI-GAN MLX inpainting requires an SnapForge-compatible mlx-lama-swift bridge on this Apple Silicon server.",
            [{"label": "build mlx-lama-swift bridge and put snapforge-mlx-inpaint or mlx-lama-serve on PATH", "op": "dependency", "package": "mlx_lama_swift"}],
        ),
        (
            r"mlx-ddcolor-swift|snapforge-mlx-colorize|mlx-ddcolor-serve|DDColor MLX models require",
            "DDColor MLX colorization requires an SnapForge-compatible mlx-ddcolor-swift bridge on this Apple Silicon server.",
            [{"label": "build mlx-ddcolor-swift bridge and put snapforge-mlx-colorize or mlx-ddcolor-serve on PATH", "op": "dependency", "package": "mlx_ddcolor_swift"}],
        ),
        (

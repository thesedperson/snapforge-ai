...
def _package_installed_from_probe(name: str, probe: dict) -> bool:
    ...
    if name == "mlx_lama_swift":
        return bool(
            (binaries.get("snapforge-mlx-inpaint") or binaries.get("mlx-lama-serve"))
            and (files.get("mlx.metallib") or files.get("default.metallib"))
        )
    if name == "mlx_ddcolor_swift":
        return bool(
            (binaries.get("snapforge-mlx-colorize") or binaries.get("mlx-ddcolor-serve"))
            and (files.get("mlx.metallib") or files.get("default.metallib"))
        )
    if name == "diffusers":
        return bool(
...
def _package_status_note(name: str, probe: dict) -> str:
    ...
    if name == "mlx_lama_swift":
        if _package_installed_from_probe(name, probe):
            found = [
                binaries.get("snapforge-mlx-inpaint"),
                binaries.get("mlx-lama-serve"),
            ]
            return f"LaMa/MI-GAN Swift MLX runner: {next((p for p in found if p), 'available')}"
        if binaries.get("snapforge-mlx-inpaint") or binaries.get("mlx-lama-serve"):
            return "LaMa/MI-GAN Swift runner is installed, but mlx.metallib is missing next to the runner."
        return "LaMa/MI-GAN inpainting models need an SnapForge-compatible mlx-lama-swift bridge on an Apple Silicon Mac."
    if name == "mlx_ddcolor_swift":
        if _package_installed_from_probe(name, probe):
            found = [
                binaries.get("snapforge-mlx-colorize"),
                binaries.get("mlx-ddcolor-serve"),
            ]
            return f"DDColor Swift MLX runner: {next((p for p in found if p), 'available')}"
        if binaries.get("snapforge-mlx-colorize") or binaries.get("mlx-ddcolor-serve"):
            return "DDColor Swift runner is installed, but mlx.metallib is missing next to the runner."
        return "DDColor colorization models need an SnapForge-compatible mlx-ddcolor-swift bridge on an Apple Silicon Mac."
    if name in dists:
...
def _package_pip_update_status(
    ...
    if pkg.get("kind") == "system" or not pkg.get("pip"):
        return PackageUpdateStatus(
            False, "Update this system dependency outside SnapForge."
        )
    ...
    if name == "vllm" and binaries.get("vllm") and not dists.get("vllm"):
        return PackageUpdateStatus(
            False,
            "Using a vLLM CLI on PATH without Python package metadata; update it outside SnapForge.",
        )
...
bin_names={{
    ...
    'mlx_lama_swift':['snapforge-mlx-inpaint', 'mlx-lama-serve'],
    'mlx_ddcolor_swift':['snapforge-mlx-colorize', 'mlx-ddcolor-serve'],
    'tmux':['tmux'],
}}
...
TMUX_LOG_DIR = Path(tempfile.gettempdir()) / "snapforge-tmux"
...
async def _generate_tmux(cmd: str, request: Request):
    ...
    script_path.write_text(
        f"#!/bin/bash\n"
        f'SNAPFORGE_USER_SHELL="${{SHELL:-}}"\n'
        f'if [ -n "$SNAPFORGE_USER_SHELL" ] && [ -x "$SNAPFORGE_USER_SHELL" ]; then\n'
        f'  SNAPFORGE_USER_PATH="$("$SNAPFORGE_USER_SHELL" -ic \'printf "__SNAPFORGE_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__SNAPFORGE_PATH__//p\' | tail -n 1 || true)"\n'
        f'  if [ -n "$SNAPFORGE_USER_PATH" ]; then export PATH="$SNAPFORGE_USER_PATH:$PATH"; fi\n'
        f"fi\n"
    ...
    async def list_packages(
        ...
        packages = [
            ...
            },
            {
                ...
                "category": "Image",
                "target": "remote",
                "install_hint": "Build an SnapForge-compatible mlx-lama-swift bridge on the selected Apple Silicon Mac and put snapforge-mlx-inpaint or mlx-lama-serve on PATH. Upstream currently ships Swift libraries plus smoke executables, not a stable image-edit CLI.",
            },
            {
                ...
                "category": "Image",
                "target": "remote",
                "install_hint": "Build an SnapForge-compatible mlx-ddcolor-swift bridge on the selected Apple Silicon Mac and put snapforge-mlx-colorize or mlx-ddcolor-serve on PATH. Upstream currently ships Swift libraries plus smoke executables, not a stable colorize CLI.",
            },
            {
                "name": "mlx_vlm",
                "pip": "mlx-vlm",

...
SNAPFORGE_MAIL_ORIGIN = "snapforge-ui"
...
def _parse_list_unsubscribe_header(value: str | None) -> list[dict]:
    ...
    first-pass SnapForge flow. HTTP unsubscribe links are useful evidence but
...
def _apply_snapforge_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-SnapForge-Origin"] = SNAPFORGE_MAIL_ORIGIN
    if kind:
        msg["X-SnapForge-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-SnapForge-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _fixture_email_record(row: dict, uid_num: int, owner: str) -> dict:
        ...
        return {
            "uid": uid,
            "message_id": f"<fixture-email-{uid}-{owner_key}@fixtures.snapforge.local>",
            "subject": subject,
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the SnapForge marker header, but include the subject
                # fallback too. The fallback uses a distinct SnapForge prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-SnapForge-Kind "reminder" SUBJECT "Reminder (SnapForge):"{from_clause})',
                )
    ...
    def execute_unsubscribe(data: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            msg_out["Message-ID"] = email.utils.make_msgid(domain="snapforge.local")
            _apply_snapforge_headers(msg_out, "unsubscribe", uid)
    ...
    @router.delete("/snapforge/reminders")
    async def delete_snapforge_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as SnapForge reminders."""
        if account_id:
            _assert_owns_account(account_id, owner)
        deleted = 0
        folders_checked = []
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-SnapForge-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (SnapForge):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (SnapForge):")})'))
                            # Legacy reminders created before the SnapForge
            ...
            logger.error(f"delete_snapforge_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    def _load_snapforge_attachment_source(db, kind: str, item_id: str, owner: str):
    ...
    @router.post("/compose-from-snapforge")
    async def compose_from_snapforge(data: dict, owner: str = Depends(require_owner)):
        """Stage an SnapForge document or gallery image as a compose upload."""
        ...
        try:
            ...
            db = _SL()
            try:
                src = _load_snapforge_attachment_source(db, kind, item_id, owner)
                if "path" in src:
            ...
            logger.error(f"Failed to stage SnapForge attachment {kind}/{item_id}: {e}")
            return {"success": False, "error": "Mail operation failed"}

    @router.post("/compose-from-snapforge-zip")
    async def compose_from_snapforge_zip(data: dict, owner: str = Depends(require_owner)):
        """Stage several SnapForge documents/gallery images as one zip attachment."""
        ...
        try:
            ...
            try:
                ...
                return _stage_compose_bytes("snapforge-attachments.zip", content)
            finally:
                db.close()
        except HTTPException:
            raise
        except Exception as e:
            logger.error(f"Failed to stage SnapForge zip attachment: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", snapforge_kind=None, snapforge_ref=None,
    ):
        ...
        _apply_snapforge_headers(outer, snapforge_kind or "scheduled", snapforge_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, snapforge_kind, owner)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("snapforge_kind") or "scheduled",
                owner or "",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="snapforge.local")

        if req.in_reply_to:
            outer["In-Reply-To"] = req.in_reply_to
        if req.references:
            outer["References"] = req.references
        if req.snapforge_kind:
            _apply_snapforge_headers(outer, req.snapforge_kind)

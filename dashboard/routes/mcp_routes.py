...
def _mcp_oauth_base_dir() -> Path:
    """Directory that may contain OAuth files managed by SnapForge."""
    ...
    async def oauth_callback(code: str, state: str, request: Request):
        ...
        if resolve_pending(state, code):
            return HTMLResponse(_oauth_result_page(
                ...
                "The MCP server is connecting. You can close this window and return to SnapForge.",
                success=True,
            ))
...
<html><head>
<meta charset="UTF-8"><title>Authorize — SnapForge</title>
<style>

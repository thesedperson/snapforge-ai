...
def _container_loopback_reachable(base_url: str, timeout: float = 0.2) -> bool:
    ...
    This distinguishes "a model server running alongside SnapForge in the same
...
def _rewrite_loopback_for_docker(base_url: str, *, container_local: bool = False) -> str:
    ...
    default) otherwise targets the SnapForge container itself, so the probe gets
    a connection error and the endpoint is rejected with a misleading "No
    models found for that provider/key".

    Cookbook local serves are the opposite case: SnapForge started the model
        ...
        if 300 <= r.status_code < 400:
            ...
            if loc.startswith("/login") or "/login" in loc:
                return {
                    ...
                    "error": "That is SnapForge, not a model server. Use the Ollama URL, usually http://host.docker.internal:11434/v1 in Docker.",
                }
...
def _is_mlx_deepseek_v4_shim_id(model_id: str) -> bool:
    m = str(model_id or "").lower()
    return "/.cache/snapforge/mlx-shims/deepseek-v4" in m
...
def _filter_mlx_deepseek_v4_repo_when_shimmed(model_ids):
    ...
    only works through SnapForge' sanitized local shim. Keep the shim as the
...
def setup_model_routes(model_discovery):
    ...
    def api_models(request: Request, refresh: bool = False, background: bool = False):
        ...
        # Page boot can opt out with background=false so opening SnapForge does
    ...
    def create_model_endpoint(
        ...
        # server. Cookbook local serves are launched inside SnapForge itself, so

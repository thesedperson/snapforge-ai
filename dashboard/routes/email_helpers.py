...
def _friendly_email_auth_error(protocol: str, host: str, error: object) -> str:
    ...
    if microsoft_basic_auth_failure:
        return (
            ...
            "Outlook/Office 365 IMAP/SMTP in most accounts. SnapForge "
...
# Override at deploy time via SNAPFORGE_MAIL_ATTACHMENTS_DIR. Defaults to a
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "snapforge_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN snapforge_kind TEXT")
...
_IMAP_TIMEOUT_SECONDS = _coerce_imap_timeout_seconds(os.environ.get("SNAPFORGE_IMAP_TIMEOUT_SECONDS"))
...
class SendEmailRequest(BaseModel):
    ...
    # Internal marker for SnapForge-generated mail (e.g. reminder, scheduled).
    snapforge_kind: Optional[str] = None

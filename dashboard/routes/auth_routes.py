...
SESSION_COOKIE = "snapforge_session"
...
def setup_auth_routes(auth_manager: AuthManager) -> APIRouter:
    ...
    async def test_integration_route(integration_id: str, request: Request):
        ...
        if preset == "ntfy":
            ...
            # base URL (e.g. `http://host:8091/snapforge`) — otherwise
            # the topic gets appended after the path and we publish to
            # `/snapforge/snapforge` (which ntfy 404s on). ntfy itself
            ...
            headers = {
                "Title": "SnapForge connectivity test",
            ...
            try:
                async with httpx.AsyncClient(timeout=8.0) as client:
                    r = await client.post(
                        full_url,
                        content="Connectivity test from SnapForge. If you see this on your phone, ntfy is wired up correctly.",
                        headers=headers,
                    )
                if r.is_success:

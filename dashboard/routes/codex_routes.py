...
reuse existing SnapForge helpers and enforce API-token scopes before touching
user data.
"""
...
def setup_codex_routes(
        ...
        with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            for path in sorted(root.rglob("*")):
                if path.is_dir() or "__pycache__" in path.parts or path.suffix == ".pyc":
                    continue
                zf.write(path, Path("snapforge") / path.relative_to(root))
        buf.seek(0)
        headers = {"Content-Disposition": 'attachment; filename="snapforge-codex-plugin.zip"'}
    ...
    async def codex_cookbook_output(request: Request, session_id: str, tail: int = 400):
        ...
        log_path = f"/tmp/snapforge-tmux/{session_id}.log"
        inner = (
...
def setup_claude_routes() -> APIRouter:
    ...
    def plugin_zip(request: Request):
        ...
        buf.seek(0)
        headers = {"Content-Disposition": 'attachment; filename="snapforge-claude-skill.zip"'}
        return StreamingResponse(buf, media_type="application/zip", headers=headers)

                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in SnapForge (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in SnapForge" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "snapforge_kind" if "snapforge_kind" in cols else "'scheduled' AS snapforge_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                # externally cron-driven `snapforge-mail poll-scheduled`, or
                # an admin running the CLI manually alongside the in-process
                # one despite the SNAPFORGE_INPROCESS_POLLERS=0 guidance) -
                ...
                snapforge_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-SnapForge-Origin"] = "snapforge-ui"
                outer["X-SnapForge-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", snapforge_kind or "scheduled")[:64]
                outer["X-SnapForge-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `snapforge-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `SNAPFORGE_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `snapforge-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("SNAPFORGE_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `SNAPFORGE_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (SNAPFORGE_INPROCESS_POLLERS=0); "
            "drive `snapforge-mail poll-scheduled` externally."
        )
        return
    import asyncio

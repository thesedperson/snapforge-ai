...
def _migrate_chat_messages_fts():
    ...
    try:
        ...
        try:
            conn.execute("CREATE VIRTUAL TABLE IF NOT EXISTS temp._snapforge_fts5_probe USING fts5(content)")
            conn.execute("DROP TABLE IF EXISTS temp._snapforge_fts5_probe")
        except Exception as e:
